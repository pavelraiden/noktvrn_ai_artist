...
class StockSuccessTracker:
        ...
        try:
            if os.path.exists(self.stats_file):
                ...
                return defaultdict(
                    lambda: {
                        "clips": defaultdict(
                            lambda: {"usage_count": 0, "releases": [], "metrics": []}
                        ),
            ...
            logger.error(
                f"Error loading stats file {self.stats_file}: {e}. Starting fresh.",
                exc_info=True,
            )
            return defaultdict(
                lambda: {
    ...
    def _save_stats(self):
        ...
        try:
            ...
            logger.error(
                f"Error saving stats file {self.stats_file}: {e}", exc_info=True
            )
    ...
    def log_clip_usage(self, release_id: Any, source_name: str, clip_id_or_url: str):
        ...
        logger.debug(
            f"Logging usage for clip 	{clip_id_or_url}	 from source 	{source_name}	 in release {release_id}"
        )
    ...
    def log_clip_performance(
        ...
        logger.debug(
            f"Logging performance for clip 	{clip_id_or_url}	 from source 	{source_name}	 in release {release_id}"
        )
        if (
            source_name not in self.source_data
            or clip_id_or_url not in self.source_data[source_name]["clips"]
        ):
            logger.warning(
                f"Attempted to log performance for clip 	{clip_id_or_url}	 from source 	{source_name}	 which has no usage logged. Logging usage first."
            )
        ...
        self.source_data[source_name]["clips"][clip_id_or_url]["metrics"].append(
            metric_record
        )
    ...
    def _calculate_clip_score(
        self, metrics: List[Dict[str, Any]], days: Optional[int] = None
    ) -> float:
        """Calculates a simple score based on recent metrics for a single clip."""
        score = 0
        count = 0
        cutoff_date = (
            datetime.utcnow() - timedelta(days=days) if days is not None else None
        )
        ...
        for metric in metrics:
            ...
            clip_score = (
                metric.get("likes", 0) * 0.2
                + metric.get("retention_pct", 0) * 0.5
                + metric.get("watch_time_avg_sec", 0) * 0.3
            )
            score += clip_score
            count += 1
    ...
    def _update_ranking(self, days: Optional[int] = None):
        ...
        logger.info(
            f"Updating source ranking, considering metrics from the last {days} days"
            if days
            else "Updating source ranking using all metrics"
        )
        for source_name, source_info in self.source_data.items():
            ...
            source_info["aggregated_score"] = (
                total_score / scored_clips_count if scored_clips_count > 0 else 0
            )
    ...
    def get_top_sources(self, genre: Optional[str] = None, days: int = 30) -> List[str]:
        ...
        if genre:
            logger.warning(
                f"Genre filtering (	{genre}	) is not implemented in get_top_sources."
            )

        # Sort sources by aggregated_score descending
        sorted_sources = sorted(
            self.source_data.items(),
            key=lambda item: item[1]["aggregated_score"],
            reverse=True,
        )
...
if __name__ == "__main__":
    ...
    tracker.log_clip_usage(
        release_id="release_001", source_name="pexels", clip_id_or_url="pexels_vid_123"
    )
    tracker.log_clip_usage(
        release_id="release_001", source_name="pexels", clip_id_or_url="pexels_vid_456"
    )
    tracker.log_clip_usage(
        release_id="release_002",
        source_name="pixabay",
        clip_id_or_url="pixabay_vid_abc",
    )
    tracker.log_clip_usage(
        release_id="release_002", source_name="pexels", clip_id_or_url="pexels_vid_123"
    )  # Used again

    # Log some performance (will trigger ranking update and save)
    tracker.log_clip_performance(
        "release_001",
        "pexels",
        "pexels_vid_123",
        likes=150,
        watch_time_avg_sec=45,
        retention_pct=60,
    )
    tracker.log_clip_performance(
        "release_001",
    ...
    tracker.log_clip_performance(
        ...
        "pexels_vid_123",
        likes=180,
        watch_time_avg_sec=50,
        retention_pct=65,
    )  # Performance for second usage
