...
try:
    ...
    logging.warning("Failed to import StockSuccessTracker. Performance-based source selection disabled.")
    # Define dummy tracker if import fails
    class StockSuccessTracker:
        def get_top_sources(self, genre: Optional[str] = None, days: int = 30) -> List[str]: return []
        def log_clip_usage(self, release_id: Any, source_name: str, clip_id: Any): pass
    class StockTrackerError(Exception): pass
...
try:
    ...
    class AudioAnalysisError(Exception): pass # Dummy if needed
...
def select_stock_videos(
    ...
    """
    logger.info(f"Starting video selection for release {release_id} based on audio features: {audio_features} and keywords: {query_keywords}")

    # Initialize tracker if not provided (or handle error if required)
    if tracker is None:
        logger.warning("No StockSuccessTracker provided. Performance-based source selection disabled.")
    ...
    try:
        ...
        logger.warning(f"Error getting top sources from tracker: {e}. Proceeding without preference.")
    ...
    if tempo > 140:
        base_query_parts.append(random.choice(["fast", "energetic", "dynamic", "action"]))
    elif tempo < 90:
        base_query_parts.append(random.choice(["slow", "calm", "relaxing", "ambient", "serene"]))
    else:
        base_query_parts.append(random.choice(["moderate tempo", "steady rhythm", "flowing"]))

    if energy > 0.7:
        base_query_parts.append(random.choice(["intense", "powerful", "vibrant", "bright"]))
    elif energy < 0.3:
        base_query_parts.append(random.choice(["gentle", "soft", "subtle", "dark", "muted"]))

    # Remove duplicates and join
    final_query = " ".join(list(dict.fromkeys(base_query_parts)))
    if not final_query:
        logger.warning("Could not generate a meaningful query from features/keywords. Using default 	abstract	.")
    ...
    logger.info(f"Search order based on tracker and availability: {searched_sources_order}")
    ...
    for source_name in searched_sources_order:
        ...
        try:
            ...
            if source_name == "pexels":
                 search_results = client.search_videos(query=final_query, per_page=max(num_videos * 3, 15), orientation=	landscape	)
        ...
        except (PexelsApiError, Exception) as e: # Catch specific client errors + general errors
    ...
    if not videos_from_sources:
        logger.warning(f"No videos found across sources for query: 	{final_query}	. Trying fallback queries.")
        fallback_queries = ["abstract background", "nature landscape", "city lights", "technology", "music visualization"]
        random.shuffle(fallback_queries)

        for fallback_query in fallback_queries:
            logger.info(f"Attempting fallback query: 	{fallback_query}	")
            for source_name in searched_sources_order: # Try sources in the same preferred order
                client = clients[source_name]
                logger.info(f"Searching source: {source_name} with fallback query: 	{fallback_query}	")
                try:
                    if source_name == "pexels":
                        search_results = client.search_videos(query=fallback_query, per_page=max(num_videos * 3, 15), orientation=	landscape	)
                        source_videos = search_results.get("videos", [])
                        logger.info(f"Found {len(source_videos)} videos from {source_name} using fallback.")
                        if source_videos:
                            videos_from_sources[source_name] = source_videos
                    ...
                    logger.warning(f"Error searching {source_name} with fallback query: {e}. Skipping source.")
                    continue
            if videos_from_sources: # If videos found with this fallback, stop trying other fallbacks
                break

    if not videos_from_sources:
        logger.error(f"No videos found even with fallback queries for audio features: {audio_features}")
        raise VideoSelectionError(f"Could not find any suitable videos for query 	{final_query}	 or fallbacks across available sources.")
    ...
    fallback_pool = [v for v in all_potential_videos if v[	 source	] not in preferred_sources]
    ...
    if len(combined_pool) < num_videos:
        logger.warning(f"Requested {num_videos} videos, but only {len(combined_pool)} were found/suitable. Returning all found.")
    ...
    for video in selected_videos_raw:
        ...
        hq_link_info = None
        if video_files:
            hq_link_info = max(video_files, key=lambda x: x.get(	 width	, 0) * x.get(	 height	, 0), default=None)
        ...
        if tracker and release_id and video_id:
            try:
                tracker.log_clip_usage(release_id=release_id, source_name=source_name, clip_id=video_id)
                logger.debug(f"Logged usage for clip {video_id} from {source_name} for release {release_id}")
            except StockTrackerError as e:
                 logger.warning(f"Failed to log clip usage for {video_id} from {source_name}: {e}")
             ...
             logger.warning(f"Skipping usage logging for clip {video_id}: release_id not provided.")
        elif not video_id:
             logger.warning(f"Skipping usage logging for video from {source_name}: video ID not found.")


    logger.info(f"Final selected videos ({len(final_selection)}): {[v[	 id	] for v in final_selection]}")
...
if __name__ == '__main__':
    ...
    class MockStockSuccessTracker(StockSuccessTracker):
        ...
        def get_top_sources(self, genre: Optional[str] = None, days: int = 30) -> List[str]:
            logger.info(f"[MockTracker] get_top_sources called (genre={genre}, days={days}). Returning: {self._top_sources}")
            return self._top_sources

        def log_clip_usage(self, release_id: Any, source_name: str, clip_id: Any):
            log_entry = {"release_id": release_id, "source_name": source_name, "clip_id": clip_id}
