...
This pipeline:
1. Uses the SpotifyApiClient to fetch top playlists (charts) for specified countries.
...
def transform_playlist_to_country_profile(playlist_data, country_code):
    """Transforms Spotify playlist data (representing a chart) into a country_profile update."""
    # This is a simplified transformation. A real implementation would likely aggregate
    # data from multiple playlists/sources for a more comprehensive country profile.
    profile_id = f"{country_code}_{datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}"
    ...
    genre_trends = {
        ...
        "chart_playlist_url": playlist_data.get("external_urls", {}).get("spotify"),
        "description": playlist_data.get("description"),
        "snapshot_id": playlist_data.get("snapshot_id"), # Useful for tracking changes
        "last_updated": datetime.now(timezone.utc).isoformat()
    }
    
    # Market data and demographics would likely come from other sources or require more complex analysis
...
def transform_track_data(track_item, artist_id="unknown_external_artist"):
    ...
    audio_features = {
        ...
        # Add BPM, key, energy etc. after fetching audio_features API
    }
    ...
    # Performance summary - this would be populated by a separate metrics pipeline
    performance_summary = {}

    return {
        "id": track_id,
        "artist_id": artist_id, # Need a way to link/create external artists or use a generic ID
        "title": title,
...
def run_spotify_charts_pipeline():
    ...
    for country in COUNTRIES:
        ...
        for playlist in toplists["playlists"]["items"]:
            ...
            logger.info(f"Processing playlist: {playlist.get("name")} (ID: {playlist_id})")
            
            # Transform and store country profile based on playlist info (simplified)
            ...
            for item in tracks_response["items"]:
                ...
                if track_data:
                    # Store unique tracks using ID as key to avoid duplicates across playlists/countries
                ...
                if audio_features_list:
                    for features in audio_features_list:
                        ...
                        if track_id in all_tracks_to_insert:
                            ...
                            existing_features = json.loads(all_tracks_to_insert[track_id]["audio_features"])
                            existing_features.update({
                                "bpm": features.get("tempo"),
                                "key": features.get("key"), # Numerical key, might need mapping
                                "mode": features.get("mode"), # Major/Minor
                            ...
                            all_tracks_to_insert[track_id]["audio_features"] = json.dumps(existing_features)
    ...
    try:
        with get_db_cursor(commit=True) as cursor:
            ...
            if all_country_profiles_to_insert:
                ...
                """
                # Consider using execute_batch for efficiency if psycopg2 version supports it well
                for profile_data in all_country_profiles_to_insert:
                    try:
                        cursor.execute(insert_query_cp, profile_data)
                    except Exception as insert_error:
                        logger.error(f"Error inserting country profile {profile_data.get("id")}: {insert_error}")
                ...
                for track_id, track_data in all_tracks_to_insert.items():
                    try:
                        cursor.execute(insert_query_track, track_data)
                         ...
                         logger.error(f"Error inserting track {track_id}: {insert_error}")
                         # Optionally rollback or continue
