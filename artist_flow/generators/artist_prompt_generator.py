...
class ArtistPromptGenerator:
        ...
        visual_elements = {
            "mysterious": {
                ...
                "fashion": [
                    ...
                    "masked",
                    "dark clothing",
                    "accessories that hide identity",
                ],
            },
            "cold": {
        ...
        for keyword in style_keywords:
            if keyword in self.visual_elements:
                ...
                for category in combined_elements:
                    combined_elements[category].extend(
                        self.visual_elements[keyword][category]
                    )
    ...
    def _extract_persona_traits(self, persona: str) -> List[str]:
        ...
        trait_expansions = {
            "mysterious": [
                ...
                "reveals little about true identity",
                "creates intrigue through ambiguity",
                "communicates through art rather than words",
            ],
            "energetic": [
                "brings high energy to performances",
                "creates dynamic, upbeat content",
                "maintains an active presence",
                "engages enthusiastically with audience",
            ],
            "soft": [
                "presents a gentle, approachable persona",
                "creates calming, soothing content",
                "communicates with warmth and empathy",
                "avoids harsh or aggressive elements",
            ],
            "aggressive": [
                "presents a bold, confrontational image",
                "pushes boundaries in content and style",
                "uses direct, powerful communication",
                "challenges conventions and expectations",
            ],
            "intellectual": [
                "incorporates complex themes and references",
                "engages with philosophical concepts",
                "creates thought-provoking content",
                "appeals to analytical listeners",
            ],
            "playful": [
                "incorporates humor and lightheartedness",
                "experiments with unexpected elements",
                "maintains a sense of fun and joy",
                "doesn't take themselves too seriously",
            ],
            "authentic": [
                "presents a genuine, unfiltered image",
                "shares personal experiences and emotions",
                "maintains consistency between art and persona",
                "values honesty in expression",
            ],
            "enigmatic": [
                "cultivates an air of mystery and intrigue",
                "leaves interpretation open to audience",
                "creates layered, symbolic content",
                "reveals identity gradually over time",
            ],
            "rebellious": [
                "challenges mainstream conventions",
                "pushes against established norms",
                "represents counterculture values",
                "stands for alternative perspectives",
            ],
            "spiritual": [
                "incorporates metaphysical or spiritual themes",
                "explores consciousness and meaning",
                "connects to deeper human experiences",
                "offers transcendent perspectives",
            ],
        }
        ...
        for trait in traits:
            if trait in trait_expansions:
                ...
                expansions = random.sample(
                    trait_expansions[trait], min(2, len(trait_expansions[trait]))
                )
                expanded_traits.append(
                    f"{trait.capitalize()}: {' and '.join(expansions)}"
                )
            else:
    ...
    def generate_artist_prompt(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Generating artist prompt for genre: {params.get('genre', 'Unknown')}"
        )
        ...
        if not target_audience:
            target_audience = ", ".join(
                random.sample(
                    genre_elements["audience"], min(2, len(genre_elements["audience"]))
                )
            )
        ...
        if not themes:
            themes = ", ".join(
                random.sample(
                    genre_elements["themes"], min(3, len(genre_elements["themes"]))
                )
            )
...
"""
    ...
    def generate_artist_name_prompt(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Generating artist name prompt for genre: {params.get('genre', 'Unknown')}"
        )
...
"""
    ...
    def generate_visual_identity_prompt(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Generating visual identity prompt for genre: {params.get('genre', 'Unknown')}"
        )
...
"""
    ...
    def generate_backstory_prompt(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Generating backstory prompt for genre: {params.get('genre', 'Unknown')}"
        )
...
def create_artist_prompt_generator(
    ...
    """
    return ArtistPromptGenerator(template_variety=template_variety, seed=seed)
