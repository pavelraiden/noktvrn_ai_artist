...
class TrackPromptGenerator:
                ...
                for category in combined_elements:
                    if isinstance(self.mood_elements[keyword][category], list):
                        combined_elements[category].extend(
                            self.mood_elements[keyword][category]
                        )
                    else:
                        combined_elements["description"].append(
                            self.mood_elements[keyword][category]
                        )
        ...
        if not any(combined_elements.values()):
            return {
                ...
                "musical_elements": [
                    ...
                    "fitting chord progressions",
                    "suitable melody style",
                ],
                "lyrical_themes": [
                    "emotion-appropriate topics",
                    "thematically consistent content",
                ],
                "production_techniques": [
                    "mood-enhancing production",
                    "appropriate effects",
                    "suitable mix balance",
                ],
                "audience_response": [
                    "emotional connection",
                    "mood-appropriate reaction",
                ],
            }
    ...
    def generate_track_prompt(
        self, artist_profile: Dict[str, Any], overrides: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Generating track prompt for artist: {artist_profile.get('name', 'Unknown')}"
        )
        ...
        if not themes:
            if "themes" in artist_profile:
                ...
                "thematic_elements" in artist_profile
                and "lyrical_themes" in artist_profile["thematic_elements"]
            ):
                themes = ", ".join(
                    artist_profile["thematic_elements"]["lyrical_themes"]
                )
            else:
        ...
        if not audience_emotion:
            ...
            if "audience_emotion" in genre_elements:
                audience_emotion = ", ".join(
                    random.sample(
                        genre_elements["audience_emotion"],
...
"""
        ...
        if structure_elements:
            structure_samples = random.sample(
                structure_elements, min(4, len(structure_elements))
            )
        ...
        production_techniques = random.sample(
            mood_elements["production_techniques"],
...
"""
    ...
    def generate_lyrics_prompt(
        ...
        """
        logger.info(
            f"Generating lyrics prompt for artist: {artist_profile.get('name', 'Unknown')}"
        )
...
"""
        ...
        if genre.lower() in ["trap", "dark trap", "phonk"]:
            ...
            prompt += (
                "- Create a strong, memorable chorus with potential for sing-along\n"
            )
            ...
            prompt += (
                "- Create lyrics that authentically represent the artist's style\n"
            )
...
"""
    ...
    def generate_melody_prompt(
        ...
        """
        logger.info(
            f"Generating melody prompt for artist: {artist_profile.get('name', 'Unknown')}"
        )
...
"""
        ...
        if genre.lower() in ["trap", "dark trap", "phonk"]:
            ...
            prompt += (
                "- Use melodic elements that support rather than compete with vocals\n"
            )
            ...
            prompt += (
                "- Consider how the melody will complement other musical elements\n"
            )
...
def create_track_prompt_generator(
    ...
    """
    return TrackPromptGenerator(template_variety=template_variety, seed=seed)


# Convenience function for generating track prompts
def generate_track_prompt(
    artist_profile: Dict[str, Any], overrides: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    Generate a comprehensive prompt for creating an AI-written song based on artist profile.
