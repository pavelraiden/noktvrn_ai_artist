...
def generate_artist_profile_prompt(artist_brief: Dict[str, Any]) -> str:
    ...
    """
    logger.info(f"Generating artist profile prompt for genre: {artist_brief.get('genre', 'Unknown')}")
    ...
    themes = random.sample(genre_elements["themes"], min(2, len(genre_elements["themes"])))
...
def generate_song_prompt(artist_profile: Dict[str, Any]) -> str:
    ...
    """
    logger.info(f"Generating song prompt for artist: {artist_profile.get('name', 'Unknown')}")
    ...
    selected_themes = random.sample(themes if isinstance(themes, list) else [themes], 
...
def generate_profile_cover_prompt(artist_profile: Dict[str, Any]) -> str:
    ...
    """
    logger.info(f"Generating profile cover prompt for artist: {artist_profile.get('name', 'Unknown')}")
    ...
    composition = random.sample(visual_elements["composition"], min(2, len(visual_elements["composition"])))
    textures = random.sample(visual_elements["textures"], min(2, len(visual_elements["textures"])))
...
def generate_track_cover_prompt(artist_profile: Dict[str, Any], song_theme: str) -> str:
    ...
    """
    logger.info(f"Generating track cover prompt for artist: {artist_profile.get('name', 'Unknown')}")
    ...
    mood_indicators = ["dark", "light", "energetic", "calm", "aggressive", "peaceful", 
                      "melancholic", "uplifting", "nostalgic", "futuristic", "mysterious"]
