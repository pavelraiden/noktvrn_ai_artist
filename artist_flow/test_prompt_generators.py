...
def test_generate_artist_profile_prompt():
    ...
    for test_name, brief in test_cases:
        ...
        try:
            ...
            assert brief.get("genre", "").lower() in prompt.lower(), "Genre should be included in prompt"
...
def test_generate_song_prompt():
    ...
    for test_name, profile in test_cases:
        ...
        try:
            ...
            assert profile.get("name", "").lower() in prompt.lower(), "Artist name should be included in prompt"
            assert profile.get("genre", "").lower() in prompt.lower(), "Genre should be included in prompt"
...
def test_generate_profile_cover_prompt():
    ...
    for test_name, profile in test_cases:
        ...
        try:
            ...
            assert "not show a realistic human face" in prompt.lower() or "do not show a realistic human face" in prompt.lower(), "Prompt should specify not to show a real human face"
...
def test_generate_track_cover_prompt():
    ...
    for test_name, test_data in test_cases:
        logger.info(f"Running test: {test_name}")
        try:
            prompt = generate_track_cover_prompt(test_data["profile"], test_data["song_theme"])
            ...
            assert test_data["profile"].get("name", "").lower() in prompt.lower(), "Artist name should be included in prompt"
            assert test_data["profile"].get("genre", "").lower() in prompt.lower(), "Genre should be included in prompt"
            assert test_data["song_theme"].lower() in prompt.lower(), "Song theme should be included in prompt"
...
def run_all_tests():
    ...
    logger.info(f"- generate_artist_profile_prompt: {'PASSED' if artist_profile_test else 'FAILED'}")
    logger.info(f"- generate_song_prompt: {'PASSED' if song_test else 'FAILED'}")
    logger.info(f"- generate_profile_cover_prompt: {'PASSED' if profile_cover_test else 'FAILED'}")
    logger.info(f"- generate_track_cover_prompt: {'PASSED' if track_cover_test else 'FAILED'}")
    
    all_passed = artist_profile_test and song_test and profile_cover_test and track_cover_test
