...
class AssetManager:
    ...
    def __init__(
        self,
        base_dir: str = "/tmp/artist_flow/assets",
    ...
    def create_asset_bundle(
        ...
        if not bundle_name:
            ...
            bundle_name = f"{artist_name}_bundle_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def load_asset_bundle(
        ...
        if not session_id:
            ...
            if len(parts) > 1:
                ...
                logger.error(f"Could not extract session ID from bundle ID: {bundle_id}")
                return None
