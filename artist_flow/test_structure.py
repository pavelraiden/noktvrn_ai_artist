...
try:
    ...
    from artist_flow.mocks.music_generator import (
        MockMusicGenerator,
        create_music_generator,
    )
    from artist_flow.mocks.image_generator import (
        MockImageGenerator,
...
def run_all_tests():
    ...
    logger.info(
        f"- Instantiation test: {'PASSED' if instantiation_success else 'FAILED'}"
    )
    logger.info(f"- Factory function test: {'PASSED' if factory_success else 'FAILED'}")
    logger.info(
        f"- Basic functionality test: {'PASSED' if functionality_success else 'FAILED'}"
    )
