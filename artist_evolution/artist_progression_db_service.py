...
def apply_progression_schema():
    ...
    try:
        ...
        with get_db_cursor(commit=True) as cursor:
            if cursor is None:
                raise ProgressionDbError("DB Cursor not available")
        ...
        logger.error(
            f"Error applying artist_progression_log schema: {e}", exc_info=True
        )
    ...
    try:
        ...
        with get_db_cursor(commit=True) as cursor:
            if cursor is None:
                raise ProgressionDbError("DB Cursor not available")
            cursor.execute(
                sql, (artist_id, event_description, performance_summary, snapshot_json)
            )
            log_id = cursor.fetchone()[0]
            logger.info(
                f"Added artist progression log entry with ID: {log_id} for artist_id: {artist_id}"
            )
            return log_id
    except Exception as e:
        logger.error(
            f"Error adding progression log for artist_id {artist_id}: {e}",
            exc_info=True,
        )
