...
from artist_builder.schema.schema_defaults import (
    ...
    get_default_subgenres,
    get_default_personality_traits,
    get_default_target_audience,
)
...
class ArtistProfileComposer:
    ...
    def compose_profile(
        ...
        new_schema_profile = self._create_schema_profile(
            formatted_legacy_profile, prompt, session_id, metadata
        )
        ...
        if not is_valid:
            logger.warning(
                f"Generated profile does not fully conform to schema: {errors}"
            )
        ...
        logger.info(
            f"Composed profile for artist '{new_schema_profile['stage_name']}' with genre '{new_schema_profile['genre']}'"
        )
    ...
    def _create_schema_profile(
        ...
        if (
            not profile.get("personality_traits")
            or len(profile["personality_traits"]) < 2
        ):
            profile["personality_traits"] = self._extract_personality_traits(
                prompt, profile["genre"]
            )
    ...
    def _fix_validation_issues(
        self, profile: Dict[str, Any], errors: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for error in errors:
            ...
            if "field required" in error.lower():
                ...
                if field == "artist_id" and not fixed_profile.get("artist_id"):
                    ...
                    fixed_profile["stage_name"] = fixed_profile.get(
                        "name", f"Artist_{random.randint(1000, 9999)}"
                    )
                    ...
                    fixed_profile["subgenres"] = get_default_subgenres(
                        fixed_profile.get("genre", self.default_genre)
                    )

                elif field == "style_description" and not fixed_profile.get(
                    "style_description"
                ):
                    genre = fixed_profile.get("genre", self.default_genre)
                    fixed_profile["style_description"] = (
                        f"A {genre} artist with a unique sound that blends traditional elements with modern production techniques."
                    )

                elif field == "voice_type" and not fixed_profile.get("voice_type"):
                    genre = fixed_profile.get("genre", self.default_genre)
                    fixed_profile["voice_type"] = (
                        f"Distinctive {genre.lower()} vocal style that resonates with listeners."
                    )

                elif field == "personality_traits" and not fixed_profile.get(
                    "personality_traits"
                ):
                    fixed_profile["personality_traits"] = (
                        get_default_personality_traits(
                            fixed_profile.get("genre", self.default_genre)
                        )
                    )

                elif field == "target_audience" and not fixed_profile.get(
                    "target_audience"
                ):
                    fixed_profile["target_audience"] = get_default_target_audience(
                        fixed_profile.get("genre", self.default_genre)
                    )

                elif field == "visual_identity_prompt" and not fixed_profile.get(
                    "visual_identity_prompt"
                ):
                    genre = fixed_profile.get("genre", self.default_genre)
                    fixed_profile["visual_identity_prompt"] = (
                        f"Professional portrait photograph of a {genre} artist with a distinctive style and presence."
                    )

                elif field == "song_prompt_generator" and not fixed_profile.get(
                    "song_prompt_generator"
                ):
                    fixed_profile["song_prompt_generator"] = self._create_song_prompt(
                        fixed_profile
                    )

                elif field == "video_prompt_generator" and not fixed_profile.get(
                    "video_prompt_generator"
                ):
                    fixed_profile["video_prompt_generator"] = self._create_video_prompt(
                        fixed_profile
                    )

                elif field == "creation_date" and not fixed_profile.get(
                    "creation_date"
                ):
                ...
                # Handle nested settings fields
                    ...
                    if not fixed_profile.get("settings"):
                        ...
                        self._fix_nested_settings(
                            fixed_profile["settings"], nested_field
                        )
            ...
            # Handle empty lists
                ...
                if field == "subgenres" and (
                    not fixed_profile.get("subgenres")
                    or len(fixed_profile["subgenres"]) == 0
                ):
                    fixed_profile["subgenres"] = get_default_subgenres(
                        fixed_profile.get("genre", self.default_genre)
                    )

                elif field == "personality_traits" and (
                    not fixed_profile.get("personality_traits")
                    or len(fixed_profile["personality_traits"]) == 0
                ):
                    fixed_profile["personality_traits"] = (
                        get_default_personality_traits(
                            fixed_profile.get("genre", self.default_genre)
    ...
    def _create_song_prompt(self, profile: Dict[str, Any]) -> str:
        ...
        style_elements = (
            re.sub(r"[.!?].*", "", style_desc).strip() if style_desc else ""
        )
    ...
    def _create_video_prompt(self, profile: Dict[str, Any]) -> str:
        ...
        visual_elements = (
            re.sub(r"[.!?].*", "", visual_prompt).strip() if visual_prompt else ""
        )

        prompt = (
            f"Create a music video teaser for a {genre} track with a {vibe} atmosphere"
        )
        ...
        if backstory_sentences:
            ...
            if sentences:
                components["backstory"] = (
                    ". ".join(sentences[: min(2, len(sentences))]) + "."
                )
