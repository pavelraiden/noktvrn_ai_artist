...
class StorageManager:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        logger.info(f"Initialized StorageManager with storage directory: {self.storage_dir}")
    ...
    def save_profile(self, profile_data: Dict[str, Any], create_backup: bool = True) -> str:
        """
            ...
            for filename in os.listdir(self.storage_dir):
                if filename.endswith(".json") and os.path.isfile(os.path.join(self.storage_dir, filename)):
                    try:
                        ...
                        # Create summary
                        summary = {
                            "artist_id": profile_data.get("artist_id", os.path.splitext(filename)[0]),
        ...
        try:
            ...
            for profile_summary in all_profiles:
                try:
                    ...
                    logger.warning(f"Error processing profile {profile_summary['artist_id']} during search: {e}")
    ...
    def update_profile(self, profile_id: str, updates: Dict[str, Any], create_backup: bool = True) -> Dict[str, Any]:
        """
    ...
    def export_profile(self, profile_id: str, export_format: str = "json", export_path: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if export_path is None:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                export_path = os.path.join(export_dir, f"{profile_id}_{timestamp}.{export_format}")
            ...
            if export_format.lower() == "json":
                ...
                try:
                    ...
                    export_path = export_path.replace(".yaml", ".json").replace(".yml", ".json")
                    with open(export_path, 'w') as f:
                        json.dump(profile_data, f, indent=2)
            else:
                logger.warning(f"Unsupported export format: {export_format}, falling back to JSON")
    ...
    def import_profile(self, import_path: str, overwrite_existing: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "artist_id" not in profile_data:
                ...
                logger.info(f"Added missing artist_id to imported profile: {profile_data['artist_id']}")
            ...
            for profile in existing_profiles:
                if profile["artist_id"] == profile_data["artist_id"]:
                    if not overwrite_existing:
                        logger.error(f"Profile with ID {profile_data['artist_id']} already exists and overwrite_existing is False")
                        raise StorageError(f"Profile with ID {profile_data['artist_id']} already exists")
                    else:
                        logger.warning(f"Overwriting existing profile with ID {profile_data['artist_id']}")
...
def main():
    ...
    try:
        ...
        updated_profile = storage_manager.update_profile(profile_data["artist_id"], updates)
