    ...
    for field in LIST_FIELDS:
        if field in input_data and not isinstance(input_data[field], list):
            ...
            if isinstance(input_data[field], str):
                try:
                    input_data[field] = [item.strip() for item in input_data[field].split(",")]
                except Exception:
                    errors.append(f"Field {field} must be a list or comma-separated string")
            else:
...
def normalize_input(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "personality_traits" in normalized and isinstance(normalized["personality_traits"], str):
...
def collect_cli_input() -> Dict[str, Any]:
    ...
    parser.add_argument("--subgenres", required=True, help="Comma-separated list of subgenres")
    parser.add_argument("--style-description", required=True, help="Description of the artist's style")
    parser.add_argument("--voice-type", required=True, help="Description of the artist's voice")
    parser.add_argument("--personality-traits", required=True, help="Comma-separated list of personality traits")
    parser.add_argument("--target-audience", required=True, help="Description of the target audience")
    parser.add_argument("--visual-identity-prompt", required=True, help="Prompt for generating visual identity")
    
    # Add arguments for optional fields
    parser.add_argument("--real-name", help="Artist's real name (if different from stage name)")
    parser.add_argument("--backstory", help="Artist's fictional backstory")
    parser.add_argument("--influences", help="Comma-separated list of musical influences")
    parser.add_argument("--notes", help="Additional notes about the artist")
    parser.add_argument("--song-prompt-generator", help="Template to use for song prompt generation")
    parser.add_argument("--video-prompt-generator", help="Template to use for video prompt generation")
...
def process_input(input_data: Dict[str, Any], input_source: str = "unknown") -> Dict[str, Any]:
    """
