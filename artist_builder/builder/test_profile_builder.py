...
from builder.error_handler import ArtistBuilderError, InputError, ValidationError, StorageError
...
class TestArtistProfileBuilder(unittest.TestCase):
    ...
    def test_get_artist_profile(self):
        ...
        # Get the profile
        retrieved_profile = self.builder.get_artist_profile(created_profile["artist_id"])
    ...
    def test_update_artist_profile(self):
        ...
        updated_profile = self.builder.update_artist_profile(created_profile["artist_id"], updates)
    ...
    def test_apply_trend_analysis(self):
        ...
        updated_profile = self.builder.apply_trend_analysis(created_profile["artist_id"], trend_data)
        
        # Verify profile was updated
        self.assertIsNotNone(updated_profile)
        self.assertEqual(updated_profile["artist_id"], created_profile["artist_id"])
        
        logger.info(f"Applied trend analysis to test profile: {updated_profile['artist_id']}")
    ...
    def test_adapt_behavior(self):
        ...
        updated_profile = self.builder.adapt_behavior(created_profile["artist_id"], performance_data)
        
        # Verify profile was updated
        self.assertIsNotNone(updated_profile)
        self.assertEqual(updated_profile["artist_id"], created_profile["artist_id"])
        
        logger.info(f"Adapted behavior for test profile: {updated_profile['artist_id']}")
    ...
    def test_full_workflow(self):
        ...
        updated_profile = self.builder.update_artist_profile(profile["artist_id"], updates)
        
        # Apply trend analysis
        trend_data = {
            "trending_subgenres": ["Darksynth"],
            "genre_compatibility": {
                "Electronic": {"Darksynth": 0.9}
            }
        }
        trend_profile = self.builder.apply_trend_analysis(profile["artist_id"], trend_data)
        
        # Apply behavior adaptation
        performance_data = {
            "trait_performance": {"Creative": 0.85}
        }
        behavior_profile = self.builder.adapt_behavior(profile["artist_id"], performance_data)
