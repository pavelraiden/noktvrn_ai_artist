...
try:
    ...
    class LLMOrchestrator:
        def __init__(self, primary_model="mock_primary", fallback_model="mock_fallback"):
            self.primary_model = primary_model
            self.fallback_model = fallback_model
            logging.info(f"Initialized Mock LLMOrchestrator (primary: {primary_model}, fallback: {fallback_model})")
        ...
        async def generate_profile(self, input_data: Dict[str, Any], model: str) -> Dict[str, Any]:
            ...
            profile.update({
                ...
                "stage_name": profile.get("stage_name", "Generated Artist"),
                "genre": profile.get("genre", "Electronic"),
                "creation_timestamp": datetime.now().isoformat(),
                "creation_method": f"llm_pipeline (model: {model})",
                "biography": f"Generated biography via {model} for {profile["stage_name"]}.",
                "influences": ["Mock Influence 1", "Mock Influence 2"],
                "social_media": {"twitter": f"@{profile["stage_name"].lower().replace(" ", "_")}"},
                "discography": [],
                "upcoming_releases": [],
                "visual_identity": {"color_palette": ["#ABCDEF"], "style_keywords": ["mock"]},
                "metadata": {"version": "1.1", "generated_by": f"mock_llm_orchestrator ({model})"}
            })
            logging.info(f"Mock LLMOrchestrator successfully generated profile using model: {model}")
            return profile
...
LLM_RETRY_EXCEPTIONS = (LLMOrchestratorError, ConnectionError, TimeoutError, asyncio.TimeoutError)
...
class ArtistProfileLLMPipeline:
    ...
    def __init__(self, primary_model: str = "gpt-4-turbo", fallback_model: str = "gpt-3.5-turbo"):
        """Initialize the LLM pipeline with primary and fallback model preferences."""
        self.primary_model = primary_model
        self.fallback_model = fallback_model
        # Instantiate the orchestrator (real or mock)
        self.orchestrator = LLMOrchestrator(primary_model=self.primary_model, fallback_model=self.fallback_model)
        logger.info(f"Initialized ArtistProfileLLMPipeline (Primary: {self.primary_model}, Fallback: {self.fallback_model})")
    ...
    async def _attempt_generation(self, input_data: Dict[str, Any], model: str) -> Dict[str, Any]:
        ...
        try:
            ...
            profile = await asyncio.wait_for(self.orchestrator.generate_profile(input_data, model), timeout=120.0)
            ...
            logger.error(f"Unexpected error during generation with model {model}: {e}", exc_info=True)
            # Wrap unexpected errors
            raise LLMPipelineError(f"Unexpected error with model {model}: {e}") from e

    async def generate_complete_profile(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Starting profile generation for input: {json.dumps(input_data)[:100]}...")
        
        # --- Try Primary Model --- #
        try:
            logger.info(f"Attempting generation with primary model: {self.primary_model}")
            # Use await here as _attempt_generation is now async
            generated_profile = await self._attempt_generation(input_data, self.primary_model)
            logger.info("Successfully generated profile using primary model.")
            return generated_profile
        except Exception as primary_error:
            logger.warning(f"Primary model ({self.primary_model}) failed after retries: {primary_error}. Attempting fallback...")

            # --- Try Fallback Model --- #
            try:
                logger.info(f"Attempting generation with fallback model: {self.fallback_model}")
                # Use await here as _attempt_generation is now async
                generated_profile = await self._attempt_generation(input_data, self.fallback_model)
                ...
                generated_profile["metadata"]["primary_model_error"] = str(primary_error)
                return generated_profile
            except Exception as fallback_error:
                logger.error(f"Fallback model ({self.fallback_model}) also failed after retries: {fallback_error}")
...
async def main():
    ...
    pipeline = ArtistProfileLLMPipeline(primary_model="mock_primary", fallback_model="mock_fallback")
    ...
    pipeline_fail_primary = ArtistProfileLLMPipeline(primary_model="mock_primary_fail", fallback_model="mock_fallback_ok")
    ...
    pipeline_fail_all = ArtistProfileLLMPipeline(primary_model="mock_primary_fail", fallback_model="mock_fallback_fail")
