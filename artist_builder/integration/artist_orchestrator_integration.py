...
from llm_orchestrator.orchestrator import Orchestrator, OrchestratorFactory, OrchestrationResult, OrchestrationStatus
...
class ArtistPromptOrchestrator:
    ...
    def __init__(
        ...
        self.orchestrator = orchestrator or OrchestratorFactory.create_default_orchestrator(
    ...
    async def create_artist_prompt(
        self,
        artist_parameters: Dict[str, str],
        ...
        initial_prompt_package = self.prompt_designer.generate_prompt_for_review(artist_parameters)
        ...
        result = await self.orchestrator.orchestrate_prompt(
            ...
            parameters={"artist_parameters": artist_parameters}
        )
        ...
        logger.info(f"Created artist prompt with confidence score: {result.confidence_score}")
    ...
    def _extract_final_prompt(self, orchestration_content: str) -> str:
        ...
        for line in lines:
            ...
            if "final prompt" in line.lower() or "improved prompt" in line.lower() or "refined prompt" in line.lower():
        ...
        if not prompt_lines:
            ...
            for paragraph in paragraphs:
                ...
                if len(paragraph) > len(candidate_prompt) and not any(x in paragraph.lower() for x in ["feedback", "review", "suggestion", "improve"]):
