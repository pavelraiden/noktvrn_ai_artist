...
from artist_builder.validators.feedback_loop_checker import (
    FeedbackLoopChecker,
    FeedbackLoopStatus,
)
...
class TestArtistCreationFlow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.prompt_validator = PromptValidator(min_length=50, confidence_threshold=0.7)
        self.feedback_loop_checker = FeedbackLoopChecker(
            max_iterations=3, success_threshold=0.8
        )
        self.review_logger = ReviewLogger(storage_dir=self.temp_dir)
    ...
    def test_artist_creation_flow(self):
        ...
        prompt_package = self.prompt_designer.generate_prompt_for_review(
            artist_parameters
        )
        ...
        while self.feedback_loop_checker.should_continue() and iteration <= 3:
            ...
            # Validate the prompt
            result, confidence_score, feedback = self.prompt_validator.validate_prompt(
                current_prompt
            )
            ...
            if (
                ...
                or not self.feedback_loop_checker.should_continue()
            ):
        ...
        if final_prompt is None:
            ...
            if best_iteration:
                ...
                matching_prompts = [
                    p
                    for p in prompts
                    if p.get("iteration") == best_iteration["iteration_number"]
                ]
                if matching_prompts:
    ...
    def _improve_prompt(self, prompt, feedback):
        ...
        if (
            "required_elements" in feedback
            and feedback["required_elements"]["status"] == "fail"
        ):
        ...
        if "length" in feedback and feedback["length"]["status"] == "fail":
            if (
                feedback["length"].get("actual_length", 0)
                < self.prompt_validator.min_length
            ):
