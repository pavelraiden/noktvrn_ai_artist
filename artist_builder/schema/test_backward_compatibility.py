...
class TestBackwardCompatibility(unittest.TestCase):
    ...
    def test_legacy_profile_conversion_roundtrip(self):
        ...
        self.assertEqual(
            roundtrip_profile["backstory"], self.legacy_profile["backstory"]
        )
        ...
        for key_tag in ["trap", "mysterious", "intense", "phantomshadow"]:
            self.assertIn(
                key_tag,
                roundtrip_profile["tags"],
                f"Key tag {key_tag} missing from roundtrip profile",
            )
    ...
    def test_composer_with_legacy_format_output(self):
        ...
        new_profile = self.composer.compose_profile(
        ...
        for field in required_fields:
            self.assertIn(
                field, legacy_profile, f"Legacy profile missing required field: {field}"
            )
    ...
    def test_metadata_preservation(self):
        ...
        self.assertEqual(
            roundtrip_profile["metadata"]["nested_field"]["inner_field"], "inner_value"
        )
