...
def legacy_to_new_schema(legacy_profile: dict) -> dict:
    ...
    """
    logger.info(
        f"Converting legacy profile '{legacy_profile.get('name', 'Unknown')}' to new schema"
    )

    # Extract existing fields
    new_profile = {
        "artist_id": legacy_profile.get(
            "id", f"artist_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ),
...
def new_to_legacy_schema(new_profile: dict) -> dict:
    ...
    """
    logger.info(
        f"Converting new schema profile '{new_profile.get('stage_name', 'Unknown')}' to legacy format"
    )
...
def combine_style_fields(legacy_profile: Dict[str, Any]) -> str:
    ...
    if not style_parts and "genre" in legacy_profile:
        style_parts.append(
            f"A {legacy_profile['genre']} artist with a distinctive sound."
        )
...
def convert_to_prompt(visual_description: str) -> str:
    ...
    if len(visual_description) > 20 and (
        "," in visual_description or "." in visual_description
    ):
        ...
        if "llm_models" in metadata:
            ...
            if isinstance(llm_models, dict):
                if "artist" in llm_models:
                    settings["llm_assignments"]["artist_prompt_llm"] = llm_models[
                        "artist"
                    ]
                if "song" in llm_models:
                    settings["llm_assignments"]["song_prompt_llm"] = llm_models["song"]
                if "video" in llm_models:
                    settings["llm_assignments"]["video_prompt_llm"] = llm_models[
                        "video"
                    ]
                if "validator" in llm_models:
                    settings["llm_assignments"]["final_validator_llm"] = llm_models[
                        "validator"
                    ]
        ...
        if "evolution" in metadata:
            ...
            if isinstance(evolution, dict):
                if "genre_shifts" in evolution:
                    settings["behavior_evolution_settings"][
                        "allow_minor_genre_shifts"
                    ] = bool(evolution["genre_shifts"])
                if "personality_shifts" in evolution:
                    settings["behavior_evolution_settings"][
                        "allow_personality_shifts"
                    ] = bool(evolution["personality_shifts"])
                if "safe_mode" in evolution:
                    settings["behavior_evolution_settings"]["safe_mode"] = bool(
                        evolution["safe_mode"]
                    )
