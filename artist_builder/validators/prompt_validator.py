...
class PromptValidator:
    ...
    def __init__(
        ...
        self.required_elements = required_elements or ["genre", "style", "vibe", "artist"]
        self.confidence_threshold = confidence_threshold
        self.style_keywords = style_keywords or [
            "unique", "creative", "authentic", "distinctive", "original",
            "innovative", "fresh", "compelling", "engaging", "captivating"
        ]
        
        logger.info("Initialized prompt validator")
    
    def validate_prompt(self, prompt: str) -> Tuple[ValidationResult, float, Dict[str, Any]]:
        """
        ...
        logger.info(f"Validated prompt with score {confidence_score:.2f}, result: {result.value}")
    ...
    def generate_feedback(self, feedback_dict: Dict[str, Any]) -> List[str]:
        ...
        if feedback_dict["required_elements"]["status"] == "fail":
            ...
            if missing:
                ...
                suggestions.append(f"Include information about the following elements: {elements_str}")
        
        # Add style suggestions
        if feedback_dict["style"]["status"] == "fail":
            suggestions.append("Enhance the creative language and use more descriptive terms")
            suggestions.append("Consider adding more unique and distinctive characteristics")
        ...
        if not suggestions:
            suggestions.append("The prompt is acceptable but could be enhanced with more vivid descriptions")
    ...
    def _check_length(self, prompt: str) -> Tuple[float, Dict[str, Any]]:
        ...
        if length < self.min_length:
            ...
            feedback["message"] = f"Prompt is too short ({length} characters). Minimum required is {self.min_length}."
            return max(0.0, length / self.min_length), feedback
        
        if length > self.max_length:
            feedback["status"] = "fail"
            feedback["message"] = f"Prompt is too long ({length} characters). Maximum allowed is {self.max_length}."
            return max(0.0, 1.0 - (length - self.max_length) / self.max_length), feedback
    ...
    def _check_style(self, prompt: str) -> Tuple[float, Dict[str, Any]]:
        ...
        if len(keywords_found) < 2:
            feedback["status"] = "fail"
            feedback["message"] = "Style lacks creativity and distinctive characteristics"
    ...
    def _check_coherence(self, prompt: str) -> Tuple[float, Dict[str, Any]]:
        ...
        if len(short_sentences) > len(sentences) * 0.3 or len(long_sentences) > len(sentences) * 0.3:
            feedback["status"] = "fail"
            feedback["message"] = "Prompt has issues with sentence structure and flow"
        
        # Calculate score based on sentence structure
        short_penalty = min(1.0, len(short_sentences) / len(sentences) * 3) if sentences else 0
        long_penalty = min(1.0, len(long_sentences) / len(sentences) * 3) if sentences else 0
