...
class FeedbackLoopChecker:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized feedback loop checker with max {max_iterations} iterations"
        )
    ...
    def track_iteration(
        ...
        logger.info(
            f"Tracked iteration {iteration_number} with score {confidence_score:.2f}, status: {self.current_status.value}"
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_score": (
                self.iterations[-1]["confidence_score"] if self.iterations else None
            ),
            "best_score": (
                max([i["confidence_score"] for i in self.iterations])
                if self.iterations
                else None
            ),
            "improvement_trend": (
                self.get_improvement_trend() if len(self.iterations) > 1 else None
            ),
        }
    ...
    def _update_status(self) -> None:
        ...
        if len(self.iterations) >= 2:
            recent_scores = [
                i["confidence_score"]
                for i in self.iterations[-self.consecutive_no_improvement - 1 :]
            ]
            improving = any(
                recent_scores[i] > recent_scores[i - 1] + self.improvement_threshold
                for i in range(1, len(recent_scores))
            )

            if (
                not improving
                and len(self.iterations) >= self.consecutive_no_improvement + 1
            ):
...
if __name__ == "__main__":
    ...
    checker.track_iteration(
        ...
        feedback={
            "improvement_suggestions": ["Add more details about the artist's style"]
        },
    )
