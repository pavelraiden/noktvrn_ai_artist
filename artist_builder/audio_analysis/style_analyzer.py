...
class StyleAnalyzer:
    ...
    def classify_genre(self, features: Dict) -> Dict:
        ...
        return {
            ...
            "all_genres": [{"genre": g, "confidence": c} for g, c in zip(sorted_genres, sorted_confidences)]
        }
    ...
    def calculate_similarity(self, features1: Dict, features2: Dict) -> float:
        ...
        if squared_diffs:
            ...
            similarity = 1.0 - min(distance, 1.0)  # Ensure similarity is between 0 and 1
        else:
            similarity = 0.0
    ...
    def analyze_trend_compatibility(self, features: Dict, trend_features: List[Dict]) -> Dict:
        """
        ...
        similarities = [self.calculate_similarity(features, tf) for tf in trend_features]
        ...
        for name, category, feature, tolerance in aspects_to_check:
            ...
            if tolerance is None:
                trend_values = [tf.get(category, {}).get(feature) for tf in trend_features]
                most_common = max(set(trend_values), key=trend_values.count) if trend_values else None
            ...
            # For numerical features
                ...
                trend_values = [tf.get(category, {}).get(feature, 0) for tf in trend_features]
        ...
        return {
            ...
            "recommendation": "High compatibility with current trends" if compatibility_score > 0.7 else
                             "Moderate compatibility with current trends" if compatibility_score > 0.4 else
                             "Low compatibility with current trends"
        }
    
    def save_as_reference(self, features: Dict, style_name: str, reference_data_path: str) -> bool:
        """
