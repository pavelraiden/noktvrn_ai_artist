...
class TestStyleAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_features = {
            ...
            "spectral": {
                ...
                "rolloff_mean": 4000.0,
                "rolloff_std": 800.0,
                "mfcc_means": [10.0, 5.0, 2.0, 1.0, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002, 0.001],
                "mfcc_stds": [2.0, 1.0, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002, 0.001, 0.0005, 0.0002]
            },
            "harmonic": {
                "key": "C",
                "key_strength": 0.7,
                "chroma_mean": [0.8, 0.2, 0.6, 0.1, 0.7, 0.3, 0.1, 0.9, 0.2, 0.5, 0.1, 0.4],
                "chroma_std": [0.2, 0.1, 0.15, 0.05, 0.18, 0.12, 0.08, 0.22, 0.1, 0.14, 0.06, 0.13],
        ...
        self.sample_features2 = {
            ...
            "spectral": {
                ...
                "rolloff_mean": 3000.0,
                "rolloff_std": 600.0,
                "mfcc_means": [8.0, 4.0, 1.5, 0.8, 0.4, 0.15, 0.08, 0.04, 0.015, 0.008, 0.004, 0.0015, 0.0008],
                "mfcc_stds": [1.5, 0.8, 0.4, 0.15, 0.08, 0.04, 0.015, 0.008, 0.004, 0.0015, 0.0008, 0.0004, 0.0001]
            },
            "harmonic": {
                "key": "A",
                "key_strength": 0.6,
                "chroma_mean": [0.5, 0.3, 0.4, 0.2, 0.5, 0.4, 0.2, 0.6, 0.3, 0.8, 0.2, 0.3],
                "chroma_std": [0.15, 0.12, 0.13, 0.08, 0.15, 0.14, 0.09, 0.16, 0.12, 0.2, 0.08, 0.1],
    ...
    def test_calculate_similarity(self):
        ...
        similarity = self.analyzer.calculate_similarity(self.sample_features, self.sample_features)
        
        # Similarity to self should be 1.0
        self.assertEqual(similarity, 1.0)
        
        # Calculate similarity between different features
        similarity = self.analyzer.calculate_similarity(self.sample_features, self.sample_features2)
    ...
    def test_analyze_trend_compatibility(self):
        ...
        compatibility = self.analyzer.analyze_trend_compatibility(self.sample_features, self.trend_features)
        ...
        compatibility_empty = self.analyzer.analyze_trend_compatibility(self.sample_features, [])
        ...
        # Test with empty features
        compatibility_empty2 = self.analyzer.analyze_trend_compatibility({}, self.trend_features)
    ...
    def test_save_as_reference(self, mock_exists, mock_json_dump, mock_file_open):
        ...
        # Test saving features as reference
        result = self.analyzer.save_as_reference(self.sample_features, "Electronic", "dummy_path.json")
