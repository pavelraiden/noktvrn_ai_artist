...
class FeatureExtractor:
        ...
        try:
            ...
            features = {
                ...
                "high_level": {
                    ...
                    "danceability": self._estimate_danceability(
                        bpm_info, tempo_analysis
                    ),
                    "mood_valence": self._estimate_mood_valence(
                        y, sr, spectrogram_features
                    ),
                    "mood_arousal": self._estimate_mood_arousal(
                        y, sr, spectrogram_features
                    ),
            ...
            return {"status": "error", "message": str(e), "filepath": filepath}
    ...
    def extract_bpm(self, y: np.ndarray, sr: int) -> Dict:
        ...
        beat_times = librosa.frames_to_time(
            beat_frames1, sr=sr, hop_length=self.hop_length
        )
    ...
    def extract_key(self, y: np.ndarray, sr: int) -> Dict:
        ...
        harmonic_ratio = np.sum(y_harmonic**2) / (
        ...
        return {
            ...
            "tonnetz_mean": [
                float(np.mean(tonnetz[i])) for i in range(tonnetz.shape[0])
            ],
    ...
    def analyze_tempo(self, y: np.ndarray, sr: int) -> Dict:
        ...
        if len(beat_frames) > 0:
            ...
            groove_intensity = float(
                np.std(beat_strengths) / np.mean(beat_strengths)
                if np.mean(beat_strengths) > 0
                else 0
            )
        else:
        ...
        if len(y) > sr * 10:  # Only for tracks longer than 10 seconds
            num_segments = min(
                4, int(len(y) / (sr * 5))
            )  # 5-second segments, max 4 segments
            segment_tempos = []
            ...
            for i in range(num_segments):
                ...
                segment = y[start:end]
                segment_tempo = librosa.beat.tempo(
            ...
            tempo_changes = {
                "detected": np.std(segment_tempos)
                > 5.0,  # Threshold for significant change
    ...
    def extract_spectrogram_features(self, y: np.ndarray, sr: int) -> Dict:
        ...
        mfcc_novelty = librosa.onset.onset_strength(
            ...
            S=librosa.power_to_db(
                librosa.feature.melspectrogram(y=y, sr=sr, hop_length=self.hop_length)
            ),
            detrend=True,
        )
        ...
        return {
            ...
            "rolloff": {"mean": float(np.mean(rolloff)), "std": float(np.std(rolloff))},
            "mfcc": {
                "means": [float(np.mean(mfccs[i])) for i in range(mfccs.shape[0])],
                "stds": [float(np.std(mfccs[i])) for i in range(mfccs.shape[0])],
            },
            "flatness": {"mean": float(np.mean(flatness))},
            "energy": {"mean": float(np.mean(rms)), "std": float(np.std(rms))},
            "zcr": {"mean": float(np.mean(zcr)), "std": float(np.std(zcr))},
            "temporal_evolution": {
    ...
    def _estimate_danceability(self, bpm_info: Dict, tempo_analysis: Dict) -> float:
        ...
        density_factor = 1.0 - min(
            1.0, abs(density - 2.0) / 2.0
        )  # Peaks at 2 onsets per second
    ...
    def _estimate_mood_valence(
        self, y: np.ndarray, sr: int, spectrogram_features: Dict
    ) -> str:
        """
        ...
        brightness = spectrogram_features["centroid"]["mean"] / (
            sr / 2
    ...
    def _estimate_mood_arousal(
        ...
        density_factor = min(
            1.0, density / 4.0
        )  # Normalize to 0-1, capping at 4 onsets per second
