...
def estimate_key(chroma, sr):
    ...
    key_strength = correlations[0][0] - correlations[1][0] if len(correlations) > 1 else correlations[0][0]
    
    # Normalize strength (optional, simple scaling)
    key_strength = max(0, min(1, key_strength * 2)) # Scale to roughly 0-1 range

    return best_key, best_mode, key_strength, correlations[:5] # Return top 5 candidates
...
def detect_rhythm_patterns(onset_env, sr, hop_length=512):
    ...
    patterns = {
        ...
        "avg_inter_onset_interval": np.mean(np.diff(onset_times)) if len(onset_times) > 1 else 0
    }
    return patterns
