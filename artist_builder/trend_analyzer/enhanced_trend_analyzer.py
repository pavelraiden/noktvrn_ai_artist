...
class EnhancedTrendAnalyzer:
    ...
    def identify_trends_by_segment(self, 
        ...
        filtered_features = self._filter_features_by_segment(features_collection, segmentation)
    ...
    def get_country_trends(self, country_code: str, time_period: str = "current") -> Dict:
        """
        ...
        country_trends = {
            ...
            "audio_feature_trends": self._extract_audio_feature_trends_from_profile(country_profile)
        }
        
        return country_trends
    
    def compare_segment_trends(self, 
        ...
        genre_comparison = self._compare_genre_trends(
            trends1.get("genre_trends", {}),
            trends2.get("genre_trends", {})
        )
        ...
        return {
            ...
            "summary": self._generate_comparison_summary(genre_comparison, feature_comparison)
        }
        ...
        while current_date <= end:
            ...
            if os.path.exists(profile_path):
                try:
                    ...
                    trend_data.append({
                        "date": date_str,
                        "genre_trends": self._extract_genre_trends_from_profile(profile),
                        "audio_feature_trends": self._extract_audio_feature_trends_from_profile(profile)
                    })
                except Exception as e:
    ...
    def predict_future_trends(self, 
        ...
        genre_predictions = self._predict_genre_trends(segment_history, prediction_periods)
        
        # Predict audio feature trends
        feature_predictions = self._predict_audio_feature_trends(segment_history, prediction_periods)
    ...
    def get_segment_trends(self, segment: Dict[str, str], time_period: str = "current") -> Dict:
        """
        ...
        if "country" in segment and "genre" not in segment and "cluster" not in segment:
            ...
            return self._get_country_genre_trends(segment["country"], segment["genre"], time_period)
        elif "country" in segment and "cluster" in segment:
            # Audience cluster trends
            return self._get_audience_cluster_trends(segment["country"], segment["cluster"], time_period)
        else:
            return {
                "status": "error",
    ...
    def _filter_features_by_segment(self, features_collection: List[Dict], segmentation: Dict[str, str]) -> List[Dict]:
        """
        ...
        for features in features_collection:
            ...
            if "country" in segmentation and features.get("metadata", {}).get("country") != segmentation["country"]:
                match = False
            
            # Example: Check genre
            if "genre" in segmentation and features.get("metadata", {}).get("genre") != segmentation["genre"]:
                match = False
            
            # Example: Check audience cluster
            if "cluster" in segmentation and features.get("metadata", {}).get("cluster") != segmentation["cluster"]:
                match = False
    ...
    def _calculate_trends(self, features_collection: List[Dict], time_period: str) -> Dict:
        """
    ...
    def _compare_genre_trends(self, trends1: Dict, trends2: Dict) -> Dict:
        ...
        for genre in all_genres:
            ...
            popularity_diff = popularity2 - popularity1 if popularity1 is not None and popularity2 is not None else None
            ...
            for feature in set(features1.keys()) | set(features2.keys()):
                ...
                mean_diff = mean2 - mean1 if mean1 is not None and mean2 is not None else None
    ...
    def _generate_comparison_summary(self, genre_comparison: Dict, feature_comparison: Dict) -> str:
        """
        ...
        if genre_comparison:
            ...
            for genre, data in genre_comparison.items():
                ...
                if popularity_diff is not None:
                    direction = "higher" if popularity_diff > 0 else "lower" if popularity_diff < 0 else "same"
        ...
        if feature_comparison:
            ...
            for feature, data in feature_comparison.items():
                ...
                if mean_diff is not None:
                    direction = "higher" if mean_diff > 0 else "lower" if mean_diff < 0 else "same"
                    summary += f"  - {feature}: {direction} in segment2\n"
        
        return summary
    
    def _analyze_trend_evolution(self, trend_data: List[Dict], segment: Dict[str, str]) -> Dict:
        """
        ...
        for genre in all_genres:
            ...
            for data_point in trend_data:
                ...
                popularity = data_point.get("genre_trends", {}).get(genre, {}).get("popularity", {}).get("stream_share")
        ...
        for feature in all_features:
            ...
            for data_point in trend_data:
                date = data_point.get("date")
                mean = data_point.get("audio_feature_trends", {}).get(feature, {}).get("mean")
    ...
    def _predict_genre_trends(self, segment_history: List[Dict], prediction_periods: int) -> Dict:
        """
    ...
    def _predict_audio_feature_trends(self, segment_history: List[Dict], prediction_periods: int) -> Dict:
        """
    ...
    def _get_country_genre_trends(self, country_code: str, genre: str, time_period: str) -> Dict:
        """
    ...
    def _get_audience_cluster_trends(self, country_code: str, cluster: str, time_period: str) -> Dict:
        """
