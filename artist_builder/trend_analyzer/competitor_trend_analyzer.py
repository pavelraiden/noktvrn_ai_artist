...
class CompetitorTrendAnalyzer:
    ...
    def __init__(
        ...
        if LLMOrchestrator:
            try:
                ...
                logger.error(
                    f"Failed to initialize LLMOrchestrator: {e}", exc_info=True
                )
                self.llm_orchestrator = None  # Disable LLM features if init fails
        else:
            logger.warning(
                "LLMOrchestrator not available. CompetitorTrendAnalyzer running without LLM support."
            )

    async def _fetch_competitor_data(
        self, artist_id: str, genre: str, country_code: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Fetches relevant competitor data from the database."""
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot fetch competitor data."
            )
            return []
        ...
        try:
            ...
            async with self.db_manager.get_connection() as conn:
                async with conn.cursor() as cur:
                    ...
                    for row in rows:
                        results.append(
                            {"competitor_id": row[0], "data": row[1]}
                        )  # Assuming analysis_data is JSONB
            logger.info(
                f"Fetched {len(results)} competitor records for genre '{genre}'"
    ...
    async def summarize_competitor_strategies(
        self, artist_id: str, genre: str, country_code: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        if not self.llm_orchestrator:
            logger.warning(
                "LLM support is disabled. Cannot summarize competitor strategies."
            )
            return None

        competitor_data = await self._fetch_competitor_data(
            artist_id, genre, country_code
        )
        ...
        **Output Requirements:** Provide a bulleted summary of the key competitor strategies and potential opportunities.
        ...
        logger.info(
            f"Requesting LLM summary for competitor strategies in genre '{genre}'..."
        )
        try:
            ...
            logger.error(
                f"LLM call failed during competitor strategy summarization: {e}",
                exc_info=True,
            )
            return None
        ...
        **Output Requirements:** Provide a concise analysis of potential market gaps and opportunities.
        ...
        try:
            gap_analysis = await self.llm_orchestrator.generate_text(
                prompt=prompt, max_tokens=768, temperature=0.7  # Adjust as needed
            )
            logger.info("Successfully generated market gap analysis.")
            return gap_analysis
        except Exception as e:
            logger.error(
                f"LLM call failed during market gap analysis: {e}", exc_info=True
            )
            return None
