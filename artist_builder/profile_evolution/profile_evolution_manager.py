...
try:
    ...
    try:
        ...
        print(
            f"Error importing DatabaseConnectionManager: {e}. Database operations will fail."
        )
...
class ProfileEvolutionManager:
    ...
    def __init__(
        ...
        if not LLMOrchestrator:
            raise ImportError(
                "LLMOrchestrator could not be imported. Profile evolution is unavailable."
            )
    ...
    async def _fetch_artist_profile(self, artist_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot fetch artist profile."
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Error fetching profile for artist {artist_id}: {e}", exc_info=True
            )
            return None

    async def _fetch_performance_metrics(
        self, artist_id: str, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot fetch performance metrics."
            )
            return []
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(metrics)} performance metrics for artist {artist_id}."
            )
            return metrics
        except Exception as e:
            logger.error(
                f"Error fetching performance metrics for artist {artist_id}: {e}",
                exc_info=True,
            )
            return []

    async def _fetch_country_trends(
        self, country_codes: List[str], limit: int = 3
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot fetch country trends."
            )
            return {}
        ...
        for country_code in country_codes:
            ...
            try:
                ...
                logger.error(
                    f"Error fetching trends for country {country_code}: {e}",
                    exc_info=True,
                )
                result[country_code] = []
    ...
    async def _fetch_evolution_history(
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot fetch evolution history."
            )
            return []
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(history)} evolution history entries for artist {artist_id}."
            )
            return history
        except Exception as e:
            logger.error(
                f"Error fetching evolution history for artist {artist_id}: {e}",
                exc_info=True,
            )
            return []

    async def _update_description_in_db(
        self, artist_id: str, new_description: str
    ) -> bool:
        """
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot update description in DB."
            )
            return False
        ...
        try:
            async with self.db_manager.get_connection() as conn:
                async with conn.cursor() as cur:
                    ...
                    if cur.rowcount > 0:
                        logger.info(
                            f"Successfully updated description for artist {artist_id}."
                        )
                        return True
                    else:
                        logger.warning(
                            f"Artist {artist_id} not found or description update failed."
                        )
                        return False
        except Exception as e:
            logger.error(
                f"Error updating description for artist {artist_id}: {e}", exc_info=True
            )
            return False
    ...
    async def _record_evolution_event(
        ...
        if not self.db_manager:
            logger.warning(
                "Database manager not provided. Cannot record evolution event."
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"Successfully recorded evolution event for artist {artist_id}."
            )
            return True
        except Exception as e:
            logger.error(
                f"Error recording evolution event for artist {artist_id}: {e}",
                exc_info=True,
            )
            return False

    async def _build_enhanced_context(
        self, artist_id: str, provided_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if metrics:
            ...
            performance_summary = {"recent_metrics": [], "top_countries": set()}
            ...
            for metric in metrics:
                ...
                if countries:
                    ...
                    top_countries = sorted(
                        countries.items(),
                        key=lambda x: x[1].get("streams", 0),
                        reverse=True,
                    )[:3]
            ...
            enhanced_context["top_countries"] = list(
                performance_summary["top_countries"]
            )

        # Fetch country trends for top countries
        if "top_countries" in enhanced_context and enhanced_context["top_countries"]:
            country_trends = await self._fetch_country_trends(
                enhanced_context["top_countries"]
            )
            if country_trends:
        ...
        logger.info(
            f"Built enhanced context for artist {artist_id} with {len(enhanced_context)} components."
        )
    ...
    async def evolve_artist_profile_description(
        ...
        """
        logger.info(
            f"Starting description evolution for artist {artist_id} with goal: {evolution_goal[:100]}..."
        )
        ...
        if not profile:
            logger.error(
                f"Could not fetch profile for artist {artist_id}. Aborting evolution."
            )
            return None
        ...
        if not current_description:
            logger.error(
                f"No description found in profile for artist {artist_id}. Aborting evolution."
            )
            return None
        ...
        if enhance_context:
            ...
            logger.info(
                f"Enhanced context built with {len(enhanced_context)} elements."
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully generated evolved description for artist {artist_id}."
            )
        except Exception as e:
            logger.error(
                f"LLM call failed during description evolution for artist {artist_id}: {e}",
                exc_info=True,
            )
            return None

        # 4. Update description in database
        update_success = await self._update_description_in_db(
            artist_id, evolved_description
        )
        if not update_success:
            logger.error(
                f"Failed to update database with evolved description for artist {artist_id}."
            )
    ...
    async def get_evolution_recommendations(
        ...
        if not context:
            logger.warning(
                f"Could not build context for artist {artist_id}. Cannot generate recommendations."
            )
            return []
        ...
        try:
            ...
            try:
                ...
                if isinstance(recommendations, list) and len(recommendations) > 0:
                    logger.info(
                        f"Generated {len(recommendations)} evolution recommendations for artist {artist_id}."
                    )
                    return recommendations
                else:
                    logger.warning(
                        f"Invalid recommendations format for artist {artist_id}."
                    )
                    return []
            except json.JSONDecodeError as e:
                logger.error(
                    f"Failed to parse recommendations JSON for artist {artist_id}: {e}"
                )
                return []

        except Exception as e:
            logger.error(
                f"LLM call failed during recommendation generation for artist {artist_id}: {e}",
                exc_info=True,
            )
            return []
...
async def main_example():
    ...
    perf_context = {
        "recent_track_performance": "moderate success in Germany",
        "audience_feedback": "lyrics are too generic",
    }
    ...
    new_desc = await profile_evolver.evolve_artist_profile_description(
        artist_id=artist_id_to_evolve, evolution_goal=goal, context=perf_context
    )
    ...
    recommendations = await profile_evolver.get_evolution_recommendations(
        artist_id_to_evolve
    )
    if recommendations:
