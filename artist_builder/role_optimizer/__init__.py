...
class RoleOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.dynamic_optimizer = RoleDynamicOptimizer(self.config.get("dynamic_optimizer_config"))
    ...
    def optimize_artist_roles(
        ...
        """
        try:
            logger.info(f"Starting role optimization for artist {artist_profile.get('stage_name', 'Unknown')}")
            ...
            result = {
                ...
                "summary": optimization_plan.get("summary", "Role optimization completed")
            }
            
            logger.info(f"Completed role optimization for artist {artist_profile.get('stage_name', 'Unknown')}")
            return result
    ...
    def get_optimization_history(
        ...
        try:
            ...
            logger.info(f"Retrieved {len(history)} optimization history entries for artist {artist_profile.get('stage_name', 'Unknown')}")
            return history
    ...
    def compare_artist_versions(
        ...
        """
        try:
            logger.info(f"Comparing artist versions for {original_profile.get('stage_name', 'Unknown')}")
            ...
            removed_roles = [role for role in original_roles if role not in updated_roles]
            
            for role in added_roles:
                comparison["role_changes"].append({
                    "type": "added",
                    "role": role,
                    "emphasis": updated_profile.get("role_emphasis", {}).get(role, "standard")
                })
            ...
            for role in removed_roles:
                comparison["role_changes"].append({
                    "type": "removed",
                    "role": role,
                    "emphasis": original_profile.get("role_emphasis", {}).get(role, "standard")
                })
            ...
            for role in common_roles:
                original_emphasis = original_profile.get("role_emphasis", {}).get(role, "standard")
                updated_emphasis = updated_profile.get("role_emphasis", {}).get(role, "standard")
            ...
            all_behaviors = set(list(original_behaviors.keys()) + list(updated_behaviors.keys()))
            ...
            logger.info(f"Completed artist version comparison with {len(comparison['role_changes'])} role changes and {len(comparison['behavior_changes'])} behavior changes")
            return comparison
