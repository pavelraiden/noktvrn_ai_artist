...
class LLMMetricsDatabase:
    ...
    def record_interaction(
        ...
        try:
            ...
            logger.debug(
                f"Recorded LLM interaction {interaction_id} for {module}.{operation}"
            )
    ...
    def record_optimization(
        ...
        try:
            ...
            logger.debug(
                f"Recorded optimization {optimization_id} of type {optimization_type}"
            )
    ...
    def calculate_aggregate_metrics(
        ...
        try:
            ...
            avg_tokens_per_interaction = (
                total_tokens / total_interactions if total_interactions > 0 else 0
            )
            ...
            prompt_completion_ratio = (
                prompt_tokens / completion_tokens if completion_tokens > 0 else 0
            )
            ...
            logger.debug(
                f"Calculated aggregate metrics for period {start_time} to {end_time}"
            )
            return result
...
class LLMEfficiencyTracker:
    ...
    def start_tracking(
        ...
        logger.debug(
            f"Started tracking LLM interaction {tracking_id} for {module}.{operation}"
        )
        return tracking_id
    ...
    def end_tracking(
        ...
        try:
            ...
            logger.debug(
                f"Ended tracking LLM interaction {tracking_id} with {result['total_tokens']} tokens"
            )
            return result
    ...
    def track_interaction(
        ...
        try:
            ...
            logger.debug(
                f"Tracked LLM interaction {interaction_id} with {result['total_tokens']} tokens"
            )
            return result
    ...
    def get_efficiency_report(
        ...
        try:
            ...
            if not start_time:
                ...
                start_time = (
                    end_time_dt.replace(hour=0, minute=0, second=0, microsecond=0)
            ...
            aggregate_metrics = self.db.calculate_aggregate_metrics(
                start_time=start_time, end_time=end_time, time_period="custom"
            )
            ...
            report = {
                ...
                "summary": {
                    ...
                    "avg_tokens_per_interaction": aggregate_metrics[
                        "avg_tokens_per_interaction"
                    ],
                    "avg_cost_per_interaction": aggregate_metrics[
                        "avg_cost_per_interaction"
                    ],
                    "avg_latency_ms": aggregate_metrics["avg_latency_ms"],
                    "total_optimizations": len(optimizations),
                    "applied_optimizations": sum(
                        1 for opt in optimizations if opt["applied"]
                    ),
                ...
                "token_usage": {
                    ...
                    "prompt_completion_ratio": aggregate_metrics[
                        "prompt_completion_ratio"
                    ],
                },
            ...
            logger.info(
                f"Generated efficiency report with {len(interactions)} interactions and {len(optimizations)} optimizations"
            )
            return report
    ...
    def get_artist_efficiency_metrics(
        ...
        try:
            ...
            # Calculate metrics
            total_tokens = sum(
                interaction["total_tokens"] for interaction in interactions
            )
            total_cost = sum(interaction["cost"] for interaction in interactions)
            successful_interactions = sum(
                1 for interaction in interactions if interaction["success"]
            )
            success_rate = (
                (successful_interactions / len(interactions)) * 100
                if interactions
                else 0
            )
            avg_tokens_per_interaction = (
                total_tokens / len(interactions) if interactions else 0
            )
            avg_cost_per_interaction = (
                total_cost / len(interactions) if interactions else 0
            )
            ...
            logger.info(
                f"Generated artist efficiency metrics for artist {artist_id} with {len(interactions)} interactions"
            )
            return result

        except Exception as e:
            logger.error(f"Error generating artist efficiency metrics: {str(e)}")
            raise LLMMetricsError(
                f"Failed to generate artist efficiency metrics: {str(e)}"
            )
    ...
    def _calculate_optimization_impact(
            ...
            if opt["improvement_percent"] is not None and opt["applied"]:
                ...
                if current_count > 1:
                    result["optimization_types"][opt_type]["avg_improvement"] = (
                        current_avg * (current_count - 1) + opt["improvement_percent"]
                    ) / current_count
                else:
                    result["optimization_types"][opt_type]["avg_improvement"] = opt[
                        "improvement_percent"
                    ]
        ...
        applied_opts = [
            opt
            for opt in optimizations
            if opt["applied"] and opt["improvement_percent"] is not None
        ]
    ...
    def _generate_efficiency_recommendations(
        ...
        for op_key, metrics in aggregate_metrics["operation_metrics"].items():
            if (
                ...
                and metrics["total_tokens"] / metrics["total_interactions"] > 3000
            ):
                high_token_operations.append(
                    {
                        "operation": op_key,
                        "avg_tokens": metrics["total_tokens"]
        ...
        if high_token_operations:
            ...
            operations_list = ", ".join(
                [
                    f"{op['operation']} ({op['avg_tokens']:.2f} tokens)"
        ...
        applied_optimization_types = set(
            opt["optimization_type"] for opt in optimizations if opt["applied"]
        )
