...
class SunoOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        connection_string_valid = (
            bas_driver_config.get("connection_string") and 
            bas_driver_config.get("connection_string") != "dummy_replace_with_real_connection_string"
        )
        ...
        if use_real_driver and connection_string_valid:
            ...
            try:
                ...
                logger.error(f"Failed to initialize REAL BAS Driver: {e}. Falling back to MockBASDriver.", exc_info=True)
                self.bas_driver_instance = MockBASDriver() # Fallback to mock
        else:
            if not use_real_driver:
                logger.warning("Configuration 'use_real_bas_driver' is false or not set. Using MockBASDriver.")
            elif not connection_string_valid:
                logger.warning("BAS Driver connection string missing, dummy, or invalid. Using MockBASDriver.")
    ...
    async def generate_song(
        ...
        try:
            ...
            effective_start_step = current_state.get("last_completed_step", -1) + 1
            logger.info(f"[{run_id}] Loaded state. Initial effective_start_step: {effective_start_step}. Run data: {current_state}")

            if effective_start_step == 0:
                logger.info(f"[{run_id}] STAGE: Translating prompt to UI actions (first run or full restart).")
                ...
                if not ui_actions:
                    logger.error(f"[{run_id}] Cannot resume run: Missing 'planned_actions' in state.")
                    raise SunoOrchestratorError(f"[{run_id}] Cannot resume run: Missing 'planned_actions' in state.")
                logger.info(f"[{run_id}] Resuming with existing planned_actions.")
            
            logger.info(f"[{run_id}] STAGE: UI Action planning complete. Total actions: {len(ui_actions)}.")

            max_retries = self.config.get("max_retries", 3)
            current_retry_attempt_count = current_state.get("current_retry_count", 0)
            ...
            while current_retry_attempt_count < max_retries and not overall_success:
                logger.info(f"[{run_id}] STAGE: Entering execution attempt {current_retry_attempt_count + 1}/{max_retries}. Current start_step for this attempt: {effective_start_step}")
                action_results = current_state.get("action_results", [])
                current_attempt_step_success = True # Tracks success of steps within this current attempt
                llm_suggested_retry_actions_for_failed_step = None # Reset for each attempt
                ...
                for i in range(effective_start_step, len(ui_actions)):
                    ...
                    logger.info(f"[{run_id}] Attempt {current_retry_attempt_count + 1}, Executing action {step_index + 1}/{len(ui_actions)}: {action.get('action')}")
                    ...
                    current_state["action_results"] = action_results # Persist results immediately

                    if not validation_result.get("approved"):
                        logger.warning(f"[{run_id}] VALIDATION FAILED for step {step_index}. Feedback: {validation_result.get('feedback')}")
                        current_attempt_step_success = False
                        llm_suggested_retry_actions_for_failed_step = await self.feedback_loop.get_retry_actions(validation_result)
                        
                        if llm_suggested_retry_actions_for_failed_step:
                            logger.info(f"[{run_id}] LLM suggested fix actions for step {step_index}. Applying and restarting inner action loop for this attempt.")
                            ...
                            ui_actions = ui_actions[:step_index] + llm_suggested_retry_actions_for_failed_step # + ui_actions[step_index+1:] if only replacing one step
                            current_state["planned_actions"] = ui_actions
                            logger.info(f"[{run_id}] New action plan has {len(ui_actions)} steps. Restarting action sequence from failed step {step_index} within current attempt.")
                            effective_start_step = step_index # Set start for the re-evaluation of the inner loop
                        else:
                            logger.error(f"[{run_id}] Validation failed for step {step_index}, and no retry actions suggested by LLM. This attempt will fail.")
                        self.state_manager.save_state(run_id, current_state) # Save state after failed step and potential action plan update
                        ...
                        self.state_manager.save_state(run_id, current_state)
                        logger.debug(f"[{run_id}] Saved state after successful step {step_index}.")
                        effective_start_step = step_index + 1 # Next step in this attempt

                # After iterating through actions for the current attempt
                if current_attempt_step_success:
                    overall_success = True
                    logger.info(f"[{run_id}] STAGE: All steps completed and validated successfully in attempt {current_retry_attempt_count + 1}.")
                else: # Current attempt failed (either a step failed validation, or LLM fix also failed)
                    current_retry_attempt_count += 1
                    current_state["current_retry_count"] = current_retry_attempt_count
                    self.state_manager.save_state(run_id, current_state)
                    logger.warning(f"[{run_id}] Execution attempt {current_retry_attempt_count}/{max_retries} (previously {current_retry_attempt_count-1}+1) failed.")

                    if current_retry_attempt_count < max_retries:
                        logger.info(f"[{run_id}] Preparing for main retry attempt {current_retry_attempt_count + 1}. Waiting for {self.config.get('retry_delay', 5)}s...")
                        ...
                        if llm_suggested_retry_actions_for_failed_step: 
                            ...
                            logger.info(f"[{run_id}] Next attempt will use LLM-modified action plan, starting from step {effective_start_step}.")
                        else:
                            # No LLM fix, or LLM fix also failed to make the step pass. Reset for a full sequence retry.
                            logger.info(f"[{run_id}] No LLM fix was applied or it failed. Resetting to attempt full sequence from beginning (step 0) for next main retry.")
                            effective_start_step = 0 
                            current_state["last_completed_step"] = -1 # Reset for full retry
                            # Optionally, re-translate prompt to actions if the original plan is suspect
                            # ui_actions = self.ui_translator.translate_prompt_to_actions(generation_prompt)
                            # current_state["planned_actions"] = ui_actions
                            self.state_manager.save_state(run_id, current_state) # Save reset state
                    else:
                        logger.error(f"[{run_id}] STAGE: All {max_retries} main retry attempts failed.")
            
            # After the while loop (retries exhausted or overall_success)
            if overall_success:
                logger.info(f"[{run_id}] STAGE: Extracting final output after successful run.")
                ...
                logger.info(f"[{run_id}] STAGE: Suno generation process completed successfully.")
                ...
                self.state_manager.save_final_state(run_id, {"final_error": error_message, "action_results": action_results}, status="failed", error=error_message)
                self.logger.end_run(run_id, {"final_error": error_message}, status="failed", error=error_message)
                logger.error(f"[{run_id}] STAGE: Suno generation process failed after all retries.")
            ...
            self.state_manager.save_final_state(run_id, {"error_details": str(e)}, status="failed", error=str(e))
            self.logger.end_run(run_id, {"error_details": str(e)}, status="failed", error=str(e))
            ...
            logger.error(error_message, exc_info=True) # Use logger.error for consistency, exc_info for stack trace
            self.state_manager.save_final_state(run_id, {"error_details": str(e)}, status="failed", error=str(e))
            self.logger.end_run(run_id, {"error_details": str(e)}, status="failed", error=str(e))
            raise SunoOrchestratorError(error_message) from e
...
async def run_real_integration_test():
    ...
    try:
        ...
        if os.path.exists(dotenv_path):
            ...
            module_logger.info(f"Loaded environment variables from: {dotenv_path}")
        else:
            module_logger.warning(f".env file not found at {dotenv_path}, relying on system environment variables.")
    except ImportError:
        module_logger.warning("python-dotenv not installed, cannot load .env file.")
    except Exception as e:
    ...
    orchestrator_config = {
        ...
        "bas_driver_config": { 
            ...
            "connection_string": os.getenv("BAS_CONNECTION_STRING", "dummy_replace_with_real_connection_string")
        }
    }
    ...
    try:
        module_logger.info(f"Calling generate_song for run_id: {test_prompt['run_id']}")
        song_metadata = await orchestrator.generate_song(test_prompt)
        module_logger.info(f"--- Test Song Generation Successful ---")
        module_logger.info(f"Song Metadata: {song_metadata.dict() if hasattr(song_metadata, 'dict') else song_metadata}")
    except SunoOrchestratorError as e:
        module_logger.error(f"--- Test Song Generation Failed ---: {e}", exc_info=True)
    except Exception as e:
        module_logger.error(f"--- Test Encountered Unexpected Error ---: {e}", exc_info=True)
