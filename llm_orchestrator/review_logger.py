...
class ReviewLogger:
    ...
    def log_prompt_version(
        ...
        logger.info(f"Logged prompt version for session {session_id}, iteration {iteration}")
        return prompt_id
    ...
    def log_validation_result(
        ...
        logger.info(f"Logged validation result for session {session_id}, iteration {iteration}")
    ...
    def log_iteration_summary(
        ...
        logger.info(f"Logged iteration summary for session {session_id}, iteration {iteration}")
        return summary_id
    ...
    def get_logs_by_session(self, session_id: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for category in logs:
            logs[category].sort(key=lambda x: (x.get("iteration", 0), x.get("timestamp", "")))
        
        logger.info(f"Retrieved {sum(len(logs.get(c, [])) for c in categories)} logs for session {session_id}")
        return logs
    ...
    def get_iteration_history(
        ...
        for summary in summaries:
            ...
            prompt_data = next((p for p in all_logs.get("prompts", []) if p.get("id") == prompt_id), None)
            feedback_data = next((f for f in all_logs.get("feedback", []) if f.get("id") == feedback_id), None)
            validation_data = next((v for v in all_logs.get("validations", []) if v.get("id") == validation_id), None)
            ...
            if validation_data:
                iteration_data["validation_result"] = validation_data.get("validation_result")
        ...
        logger.info(f"Retrieved history for session {session_id}, {'iteration ' + str(iteration) if iteration is not None else 'all iterations'}")
    ...
    def get_best_prompt(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        best_prompt = next((p for p in all_logs["prompts"] if p.get("id") == best_prompt_id), None)
...
if __name__ == "__main__":
    ...
    validation_id = logger.log_validation_result(session_id, iteration, prompt_id, validation_result)
    
    # Log iteration summary
    summary_id = logger.log_iteration_summary(
        session_id, iteration, prompt_id, feedback_id, validation_id,
        "needs_improvement", 0.65
    )
    
    # Get logs for the session
    logs = logger.get_logs_by_session(session_id)
    print(f"Retrieved {sum(len(logs.get(c, [])) for c in logs)} logs for session {session_id}")
