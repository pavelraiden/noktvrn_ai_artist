...
class Orchestrator:
    ...
    async def orchestrate_prompt(
        ...
        try:
            ...
            logger.info(f"Generating initial content for orchestration {orchestration_id}")
            ...
            while iteration < self.max_iterations:
                ...
                # Review the content
                logger.info(f"Reviewing content (iteration {iteration}) for orchestration {orchestration_id}")
                review_response, confidence_score = await self._review_content(content, parameters, result)
                
                # Check if content meets confidence threshold
                if confidence_score >= self.confidence_threshold:
                    logger.info(f"Content meets confidence threshold ({confidence_score:.2f} >= {self.confidence_threshold:.2f})")
                    break
                
                # Refine the content based on review
                logger.info(f"Refining content (iteration {iteration}) for orchestration {orchestration_id}")
                content = await self._refine_content(content, review_response, parameters, result)
            
            # Complete the orchestration
            result.complete(content, confidence_score)
            logger.info(f"Orchestration {orchestration_id} completed with confidence score {confidence_score:.2f}")
    ...
    async def _generate_content(
        ...
        # Add to history
        result.add_to_history("generate", {
...
class OrchestratorFactory:
    ...
    def create_default_orchestrator(
        ...
        generator_provider = LLMProviderFactory.create_provider("smart_mock", {
            "provider_name": "MockGenerator",
            "model_name": "mock-generator-v1"
        })
        
        reviewer_provider = LLMProviderFactory.create_provider("smart_mock", {
            "provider_name": "MockReviewer",
            "model_name": "mock-reviewer-v1"
        })
        
        refiner_provider = LLMProviderFactory.create_provider("smart_mock", {
            "provider_name": "MockRefiner",
            "model_name": "mock-refiner-v1"
        })
