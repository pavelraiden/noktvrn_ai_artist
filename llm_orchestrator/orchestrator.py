...
try:
    ...
    logger.warning("OpenAI library not found. OpenAI, DeepSeek, Grok functionality will be limited.")
...
try:
    ...
    DEFAULT_GEMINI_SAFETY_SETTINGS = {}
    logger.warning("google-generativeai library not found. Gemini functionality will be limited.")
...
try:
    ...
    logger.warning("mistralai library not found. Mistral functionality will be limited.")
...
try:
    ...
    LLM_REGISTRY = {}
    logger.warning("llm_registry.py not found or LLM_REGISTRY not defined. Auto-discovery disabled.")
...
try:
    from modules.suno.suno_orchestrator import SunoOrchestrator, SunoOrchestratorError
    from schemas.song_metadata import SongMetadata
    # Assume a default config path or mechanism for Suno Orchestrator
    DEFAULT_SUNO_CONFIG_PATH = os.path.join(PROJECT_ROOT, "config", "suno_config.json")
    # TODO: Load this config properly later
    DEFAULT_SUNO_CONFIG = {
        "state_dir": os.path.join(PROJECT_ROOT, "modules", "suno", "suno_run_states"),
        "log_dir": os.path.join(PROJECT_ROOT, "modules", "suno", "suno_run_logs"),
        "screenshot_dir": os.path.join(PROJECT_ROOT, "modules", "suno", "suno_validation_screenshots"),
        "max_retries": 3,
        "retry_delay": 5,
    ...
    logger.warning(f"Suno BAS module components not found or import error: {e}. Suno generation disabled.")


# --- Provider Configuration --- #
PROVIDER_CONFIG = {
    "openai": {
        "api_key_env": "OPENAI_API_KEY", "base_url": None, "client_class": AsyncOpenAI,
        "library_present": AsyncOpenAI is not None,
    },
    "deepseek": {
        "api_key_env": "DEEPSEEK_API_KEY", "base_url": "https://api.deepseek.com/v1", "client_class": AsyncOpenAI,
        "library_present": AsyncOpenAI is not None,
    },
    "grok": {
        "api_key_env": "GROK_API_KEY", "base_url": "https://api.x.ai/v1", "client_class": AsyncOpenAI,
        "library_present": AsyncOpenAI is not None,
    },
    "gemini": {
        "api_key_env": "GEMINI_API_KEY", "base_url": None, "client_class": None, # Special handling
        "library_present": genai is not None,
    },
    "mistral": {
        "api_key_env": "MISTRAL_API_KEY", "base_url": None, "client_class": MistralAsyncClient,
        "library_present": MistralAsyncClient is not None,
    },
    "anthropic": {
        "api_key_env": "ANTHROPIC_API_KEY", "base_url": None, "client_class": AsyncAnthropic,
...
class LLMProviderInstance:
    ...
    def __init__(self, provider: str, model_name: str, api_key: Optional[str], config: Dict[str, Any]):
    ...
    def _initialize_client(self) -> Any:
        ...
        if self.provider == "gemini":
            ...
            if not client_class: # Check if SunoOrchestrator class is available
            ...
            raise OrchestratorError(f"Client class not defined or library missing for {self.provider}")
...
class LLMOrchestrator:
    ...
    def __init__(
        ...
        if enable_auto_discovery and LLM_REGISTRY:
            ...
            for provider, data in LLM_REGISTRY.items():
                if provider in PROVIDER_CONFIG and provider != "suno": # Exclude suno from registry discovery
                    ...
                    logger.debug(f"Skipping registry provider \"{provider}\": Not in PROVIDER_CONFIG or is Suno")
    ...
    def _infer_provider(self, model_name: str) -> str:
        ...
        if "suno" in model_lower or model_lower in ["v3", "v4", "v4.5"]: # Basic Suno check
            return "suno"
        ...
        logger.warning(f"Could not infer provider for \"{model_name}\". Specify provider (e.g., \"openai:gpt-3.5-turbo\"). Defaulting to openai.")
        return "openai"
    ...
    def _add_provider(self, model_identifier: str):
        ...
        if provider_info["api_key_env"]:
            ...
            if not api_key and provider != "suno": # Suno doesn't need an API key here
                logger.warning(f"Skipping provider {provider} model {model_name}: API key ({provider_info['api_key_env']}) not found.")
                self.initialized_models.add((provider, model_name))
                return
        elif provider != "suno":
             logger.warning(f"API key environment variable not defined for provider {provider} in PROVIDER_CONFIG.")
    ...
    async def generate_text_async(
        ...
        suno_generation_prompt: Optional[Dict[str, Any]] = None # Specific prompt for Suno
    ) -> Union[str, SongMetadata, None]: # Return type can be string or SongMetadata
        """
        ...
        if not messages and prompt:
            ...
            raise ValueError("Either 'prompt' or 'messages' must be provided for LLM generation.")

        merged_params = {**self.config, **(generation_params or {})} # Merge global and local params
        ...
        for provider, model_name in self.model_preference:
            ...
            if provider == "suno":
                if not suno_generation_prompt:
                    logger.warning(f"Skipping Suno ({model_name}): Missing 'suno_generation_prompt'.")
                    last_error = OrchestratorError("Missing 'suno_generation_prompt' for Suno task.")
                    continue
                if not isinstance(instance.client, SunoOrchestrator):
                     logger.error(f"Suno provider instance is not a SunoOrchestrator: {type(instance.client)}")
                     last_error = OrchestratorError("Internal configuration error for Suno BAS.")
                     continue
                ...
                for attempt in range(self.max_retries_per_provider):
                    try:
                        ...
                        if "run_id" not in suno_generation_prompt:
                             suno_generation_prompt["run_id"] = f"suno_run_{os.urandom(4).hex()}"
                        logger.info(f"Calling Suno BAS generate_song (Run ID: {suno_generation_prompt['run_id']}) - Attempt {attempt + 1}")
                        song_metadata: SongMetadata = await suno_orchestrator.generate_song(suno_generation_prompt)
                        logger.info(f"Suno BAS generation successful for {provider}:{model_name} (Run ID: {suno_generation_prompt['run_id']})")
                        ...
                        if attempt < self.max_retries_per_provider - 1:
                            logger.info(f"Retrying Suno BAS in {current_delay:.2f} seconds...")
                            await asyncio.sleep(current_delay)
                            current_delay *= 2 # Exponential backoff
                        else:
                             logger.error(f"Suno BAS failed permanently for {provider}:{model_name} after {self.max_retries_per_provider} attempts.")
                ...
                if self.enable_fallback_notifications and provider != self.model_preference[0][0]:
                     await send_notification(f"Fallback Alert: Suno BAS ({model_name}) failed. Error: {last_error}")
            ...
            for attempt in range(self.max_retries_per_provider):
                try:
                    logger.debug(f"Calling {provider}:{model_name} (Attempt {attempt + 1}) with params: {merged_params}")
                    # --- Provider-Specific Call Logic --- #
                    if provider in ["openai", "deepseek", "grok"]:
                        if not isinstance(instance.client, AsyncOpenAI):
                             raise OrchestratorError(f"Incorrect client type for {provider}")
                        response = await instance.client.chat.completions.create(
                            model=model_name,
                            messages=self._prepare_messages(messages, system_prompt, provider),
                        ...
                        if not genai or not isinstance(instance.client, genai.GenerativeModel):
                             raise OrchestratorError(f"Incorrect client type for {provider}")
                        # Gemini uses 'contents' not 'messages'
                        gemini_contents = self._prepare_messages(messages, system_prompt, provider)
                        ...
                        if not response.candidates:
                            ...
                            block_reason = prompt_feedback.block_reason.name if prompt_feedback.block_reason else "Unknown"
                            safety_ratings = {rating.category.name: rating.probability.name for rating in prompt_feedback.safety_ratings}
                        ...
                        mistral_messages = self._prepare_messages(messages, system_prompt, provider)
                        ...
                        result = response.choices[0].message.content
                         ...
                         anthropic_messages = self._prepare_messages(messages, None, provider) # System prompt handled separately
                        ...
                        logger.error(f"Generation logic not implemented for provider: {provider}")
                        raise NotImplementedError(f"Provider {provider} not implemented")

                    logger.info(f"Generation successful using {provider}:{model_name}")
                    return result # Return the successful result

                # --- Exception Handling for LLM Providers --- #
                except (APIError, RateLimitError, MistralAPIException, AnthropicAPIError, AnthropicRateLimitError) as e:
                    last_error = e
                    logger.warning(
                    ...
                    if isinstance(e, RateLimitError) or isinstance(e, AnthropicRateLimitError) or (hasattr(e, 'status_code') and e.status_code == 429):
                        # Apply longer delay for rate limits
                        retry_delay = current_delay * 2
                        logger.info(f"Rate limit hit. Retrying in {retry_delay:.2f} seconds...")
                        await asyncio.sleep(retry_delay)
                        current_delay *= 1.5 # Slower backoff increase for rate limits
                    ...
                    if isinstance(e, google_exceptions.ResourceExhausted): # Example: Rate limit equivalent
                         retry_delay = current_delay * 2
                         logger.info(f"Resource exhausted (rate limit). Retrying in {retry_delay:.2f} seconds...")
                         await asyncio.sleep(retry_delay)
                         current_delay *= 1.5
                    elif attempt < self.max_retries_per_provider - 1:
                         logger.info(f"Retrying in {current_delay:.2f} seconds...")
                    ...
                    last_error = e
                    logger.error(f"Orchestrator error with {provider}:{model_name}: {e}")
            ...
            if provider != self.model_preference[0][0] or attempt == self.max_retries_per_provider:
                 # Check if this provider failed and we are moving to the next one
                 is_final_failure_for_provider = attempt == self.max_retries_per_provider -1
                 is_not_last_provider = (provider, model_name) != self.model_preference[-1]

                 if is_final_failure_for_provider and is_not_last_provider and self.enable_fallback_notifications:
        ...
        if self.enable_fallback_notifications:
            await send_notification(f"Critical Alert: All LLM providers failed. Last Error: {last_error}")
        ...
        if provider == "gemini":
            ...
            for msg in prepared_messages:
                ...
                if role == current_role:
                    merged_content += "\n\n" + content # Merge consecutive messages of same role
                else:
                    if current_role is not None:
                         gemini_contents.append({'role': current_role, 'parts': [{'text': merged_content}]})
                    current_role = role
            ...
            for msg in prepared_messages:
                 ...
                 if not anthropic_messages and msg["role"] == "assistant":
                     logger.warning("Anthropic messages should start with 'user' role. Prepending dummy user message.")
                     anthropic_messages.append({"role": "user", "content": "(Context starts)"}) # Or handle error
                 elif anthropic_messages and msg["role"] == last_role:
                      logger.warning(f"Anthropic messages must alternate roles. Skipping duplicate role '{msg['role']}'.")
...
async def main_example():
    ...
    fallbacks = ["mistral:mistral-large-latest", "anthropic:claude-3-haiku-20240307", "suno:v3"] # Add suno here
    ...
    logger.info("\n--- Test Case 1: Standard Text Generation ---")
    messages_test_1 = [
        {"role": "user", "content": "Write a short poem about a rainy day in a city."}
    ]
    system_prompt_test_1 = "You are a helpful assistant that writes short, creative poems."
    ...
    if isinstance(result_1, str):
        ...
        logger.error("Test Case 1 Failed: No provider could generate text.")
         ...
         logger.error(f"Test Case 1 Failed: Unexpected return type {type(result_1)}")
    ...
    suno_in_preference = any(p == 'suno' for p, m in orchestrator.model_preference)
    ...
    if suno_in_preference and suno_bas_available:
        ...
        generation_params_test_2 = {} # No specific LLM params needed for Suno call
        ...
        if isinstance(result_2, SongMetadata):
            logger.info(f"Test Case 2 Result (Suno BAS Success):\n{result_2.model_dump_json(indent=2)}")
        elif result_2 is None:
            logger.error("Test Case 2 Failed: Suno BAS generation failed or no fallback succeeded.")
        else:
             logger.error(f"Test Case 2 Failed: Unexpected return type {type(result_2)}")
    else:
        logger.warning("Skipping Test Case 2: Suno BAS not available or not configured in preference list.")
