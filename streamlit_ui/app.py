...
try:
    ...
    st.error(f"Error importing ReleaseManager: {e}. Ensure release_manager.py is in the correct path and PYTHONPATH is set.")
    ...
    class ReleaseManager:
        ...
        def get_release_status(self, release_id):
            return {"release_id": release_id, "status": "error_loading_status", "history": [], "error": "Could not load ReleaseManager"}
        
        def _get_metadata_path(self, release_id: str) -> str:
            # This won't actually be used if get_all_release_ids can't list files
            return os.path.join(self.config.get("release_metadata_dir", "./release_metadata"), f"{release_id}.json")

        def get_all_release_ids(self) -> list:
            metadata_dir = self.config.get("release_metadata_dir", "./release_metadata")
...
RELEASE_MANAGER_CONFIG = {
    "release_metadata_dir": os.path.join(PROJECT_ROOT, "release_chain", "test_release_metadata"), # Adjust if you used a different dir
    "preview_dir": os.path.join(PROJECT_ROOT, "release_chain", "test_previews"),
    "upload_dir": os.path.join(PROJECT_ROOT, "release_chain", "test_upload_ready")
}
...
st.set_page_config(page_title="AI Music Release Pipeline Status", layout="wide")

st.title("🎵 AI Music Release Pipeline Status")
st.caption(f"Monitoring releases from: {RELEASE_MANAGER_CONFIG['release_metadata_dir']}")
...
def display_release_details(release_id):
    ...
    with col1:
        st.metric("Current Status", status_data.get("status", "N/A").replace("_", " ").title())
        st.text(f"Last Updated: {status_data.get('last_updated', 'N/A')}")

    with col2:
        if status_data.get("song_metadata"):
            st.text_input("Song Title", status_data["song_metadata"].get("title", "N/A"), disabled=True)
            st.text_input("Artist", status_data["song_metadata"].get("artist", "N/A"), disabled=True)
        else:
    ...
    if history:
        for entry in reversed(history): # Show newest first
            with st.expander(f"{datetime.fromisoformat(entry['timestamp']).strftime('%Y-%m-%d %H:%M:%S')} - {entry['status'].replace('_', ' ').title()}"):
    ...
    if status_data.get("error"):
        st.markdown("---    ")
        st.error(f"Error associated with this release: {status_data.get('error')}")
...
if not all_release_ids:
    st.sidebar.info("No releases found. Ensure the metadata directory is correct and contains release files.")
    st.info("No releases found to display. Please initiate a release using the `release_manager.py` script or ensure the metadata directory is correctly configured and populated.")
else:
    selected_release_id = st.sidebar.selectbox("Select a Release ID to view details:", options=all_release_ids)
...
st.sidebar.markdown("---    ")
st.sidebar.markdown("**Note:** This UI reads data from the release metadata JSON files. Ensure the `ReleaseManager` script is run to create and update these files.")
