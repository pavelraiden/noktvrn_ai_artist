...
if os.path.exists(imagemagick_path):
    try:
        ...
        logger.warning(
            f"Failed to set ImageMagick path: {e}. Text rendering might fail if not found."
        )
else:
    logger.warning(
        f"ImageMagick binary not found at {imagemagick_path}. Text rendering might fail."
    )
...
def add_text_overlay(
    ...
    position: tuple = (
        "center",
        "bottom",
    ),  # e.g., ('center', 'bottom'), (10, 10), ('left', 50)
    start_time: float = 0,
    duration: float | None = None,
    # Margin from edge if position is like ('center', 'bottom')
    margin: int = 10,
    ...
    try:
        ...
        logger.debug(
            f"Video clip loaded: Duration={video_clip.duration}s, Size={video_clip.size}"
        )
        ...
        logger.debug(
            f"Text clip created: Duration={txt_clip.duration}s, Position=({final_pos_x}, {final_pos_y})"
        )
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(
        os.path.dirname(__file__), "..", "output", "edited_videos"
    )
