...
class LyricsService:
    ...
    def generate_lyrics(
        ...
        if not self.orchestrator:
            logger.error(
                "LLM Orchestrator not available. Cannot generate lyrics."
            )
            return None
        ...
        logger.info(
            f"Generating lyrics with LLM. Tempo: {tempo}, Duration: {duration}"
        )
        ...
        try:
            ...
            if not lyrics:
                logger.error(
                    "LLM returned empty response for lyrics generation."
                )
                return None
            # Corrected f-string: Use escaped newline \n
            logger.info(
            ...
            logger.error(
                f"Unexpected error during lyrics generation: {e}",
                exc_info=True,
            )
