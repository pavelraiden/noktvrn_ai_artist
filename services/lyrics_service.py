# Service for generating lyrics using LLM, potentially aligned with beat
# analysis
...
try:
    # Attempt to reuse the orchestrator instance if possible, otherwise create
    # new
    # This is a simplification; a better design would pass the orchestrator
    # instance
...
class LyricsService:
    ...
    def generate_lyrics(        self,         base_prompt: str,         genre: str,         style_notes: str,         llm_config: dict,         tempo: float | None = None,         duration: float | None = None,    ) -> str | None:
        ...
        prompt += (
            "Please generate suitable lyrics. Focus on evocative language "
            "and structure appropriate for the genre and duration."
        )
        ...
        try:
            response = self.orchestrator.generate_text(                prompt=prompt,                 model_name=model,                 max_tokens=max_tokens,                 temperature=temperature,            )
...
if __name__ == "__main__":
    ...
    if lyrics_service.orchestrator:
        ...
        generated_lyrics = lyrics_service.generate_lyrics(            test_base_prompt,             test_genre,             test_style,             test_config,             test_tempo,             test_duration,        )
        ...
        generated_lyrics_no_timing = lyrics_service.generate_lyrics(            test_base_prompt, test_genre, test_style, test_config        )
