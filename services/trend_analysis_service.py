...
class TrendAnalysisService:
    ...
    def get_twitter_trends(self, query: str, count: int = 20, search_type: str = 'Top'):
        ...
        try:
            ...
            if response and 'result' in response and 'timeline' in response['result']:
                logging.info(f"Successfully fetched {len(response['result']['timeline'].get('instructions', []))} instructions from Twitter.")
                return response
            else:
                logging.error(f"Unexpected response structure from Twitter API: {response}")
                return None
    ...
    def calculate_trend_score(self, twitter_data):
        ...
        """
        if not twitter_data or 'result' not in twitter_data or 'timeline' not in twitter_data['result']:
            logging.warning("Cannot calculate trend score: Invalid Twitter data provided.")
            return 0
...
if __name__ == "__main__":
    ...
    trends = trend_service.get_twitter_trends(query=search_query, count=50, search_type='Latest')
