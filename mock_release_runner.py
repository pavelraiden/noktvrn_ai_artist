        ...
        try:
            for key in path:
                if isinstance(value, dict):
                    ...
                    logger.warning(f"Could not resolve path part {joined_path_str}")
...
def substitute_dict_templates(data: Any, context: Dict[str, Any]) -> Any:
    ...
    if isinstance(data, dict):
        return {k: substitute_dict_templates(v, context) for k, v in data.items()}
...
async def run_release_plan(plan_path: str):
    ...
    # CORRECTED LINE 82:
    logger.info(f"Initializing LLM Orchestrator for release: {plan.get('release_id')}")
    global_config = plan.get("global_config", {})
    try:
        # Use primary LLM and fallbacks from the plan
        orchestrator = LLMOrchestrator(
            primary_model=global_config.get("primary_llm", "gemini:gemini-1.5-flash-latest"),
            fallback_models=global_config.get("fallback_llms", []) + [global_config.get("suno_model")], # Ensure Suno is in fallbacks if specified
            config={"temperature": 0.7}, # Basic config
            enable_fallback_notifications=False
        )
    except Exception as e:
        logger.error(f"Failed to initialize LLM Orchestrator: {e}", exc_info=True)
        return
    ...
    for i, step in enumerate(plan.get("steps", [])):
        ...
        try:
            ...
            if step_type == "llm_generate_text":
                ...
                model = step_params.get("model", global_config.get("primary_llm"))
                ...
                if isinstance(result, str):
                    ...
                    logger.warning(f"LLM step returned unexpected type: {type(result)}")

            elif step_type == "suno_bas_generate_song":
                suno_prompt = step_params.get("suno_prompt", {})
                model_identifier = suno_prompt.get("model", global_config.get("suno_model")) # Get specific suno model
                if not model_identifier or not model_identifier.startswith("suno:"):
                     logger.error(f"Invalid or missing Suno model identifier in step {step_id}")
                     raise ValueError("Suno model identifier required for suno_bas_generate_song step.")

                # Ensure run_id is passed or generated
                if "run_id" not in suno_prompt:
                    # CORRECTED LINE 144:
                    suno_prompt["run_id"] = f"{plan.get('release_id', 'mock_release')}_{step_id}_{os.urandom(3).hex()}"

                logger.info(f"Calling Suno BAS ({model_identifier}) via orchestrator.")
                logger.debug(f"Suno Prompt: {json.dumps(suno_prompt, indent=2)}")
                ...
                if isinstance(result, SongMetadata):
                    logger.info(f"Suno BAS Result: {result.model_dump_json(indent=2)}")
                else:
                    logger.warning(f"Suno BAS step returned unexpected type: {type(result)}. Expected SongMetadata.")
                ...
                result = {"status": "finalized", "inputs_received": step_params.get("inputs")}
                logger.info(f"Finalize Result: {result}")

            else:
                logger.warning(f"Unsupported step type: {step_type}. Skipping.")
                result = {"status": "skipped", "reason": f"Unsupported type: {step_type}"}

            # Store output
            output_var = step.get("output_variable")
            if output_var:
                step_outputs[step_id] = {"output": result}
                logger.debug(f"Stored output in context variable: steps.{step_id}.output")
            else:
                 step_outputs[step_id] = {"status": "completed", "result_type": type(result).__name__}
            ...
            logger.warning("Continuing to next step despite error.") # For mock test, continue
    ...
    try:
        print(json.dumps(context, indent=2, default=str)) # Use default=str for non-serializable objects
    except Exception as e:
