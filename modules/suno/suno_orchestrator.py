...
from modules.suno.suno_state_manager import (
    SunoStateManager,
...
from modules.suno.suno_ui_translator import (
    SunoUITranslator,
    SunoUITranslatorError,
    MockBASDriver,
)
from modules.suno.suno_feedback_loop import (
    SunoFeedbackLoop,
...
bas_driver = (
    MockBASDriver()
)  # KEEPING MOCK FOR NOW - NEED REAL DRIVER PATH/CLASS
...
class SunoOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if (
            ...
            logger.info(
                "Attempting to initialize REAL BAS Driver (assuming class BASDriver exists)."
            )
    ...
    async def generate_song(
        ...
        try:
            ...
            while current_retry < max_retries and not overall_success:
                logger.info(
                    f"[{run_id}] Starting execution sequence (Attempt {current_retry + 1}/{max_retries})"
                )
                ...
                for i in range(start_step, len(ui_actions)):
                    ...
                    if not validation_result.get("approved"):
                        logger.warning(
                            f"[{run_id}] Step {step_index} failed validation: {validation_result.get('feedback')}"
                        )
                        step_success = False
                        retry_actions = (
                        ...
                        if retry_actions:
                            logger.info(
                                f"[{run_id}] Applying suggested fix actions from LLM."
                            )
                            ui_actions = (
                                ui_actions[:step_index] + retry_actions
                            )
                            current_state["planned_actions"] = ui_actions
                            logger.info(
                                f"[{run_id}] New action plan: {len(ui_actions)} steps. Restarting sequence."
                            )
                            start_step = step_index
                        else:
                            logger.error(
                                f"[{run_id}] Validation failed, but no retry actions suggested. Aborting attempt."
                            )
                        break
                    else:
                        logger.info(
                ...
                if step_success:
                    ...
                    logger.info(
                        f"[{run_id}] All steps completed and validated successfully in attempt {current_retry + 1}."
                    )
                else:
                    current_retry += 1
            ...
            if overall_success:
                ...
                logger.info(
                    f"[{run_id}] Suno generation completed successfully."
                )
                ...
                self.state_manager.save_final_state(
                    run_id, None, status="failed", error=error_message
                )
                self.logger.end_run(
                    run_id, None, status="failed", error=error_message
                )
            ...
            error_message = (
                f"[{run_id}] BAS module error during Suno generation: {e}"
            )
            logger.error(error_message)
            self.state_manager.save_final_state(
                run_id, None, status="failed", error=str(e)
            )
            self.logger.end_run(run_id, None, status="failed", error=str(e))
            raise SunoOrchestratorError(error_message) from e
        except Exception as e:
            error_message = (
                f"[{run_id}] Unexpected error during Suno generation: {e}"
            )
...
async def run_real_integration_test():
    ...
    logger.info(
        f"Orchestrator Config (API keys masked): { {k: (v if k != 'llm_validator_config' else {vk: ('***' if vk == 'api_key' else vv) for vk, vv in v.items()}) for k, v in orchestrator_config.items()} }"
    )
    ...
    try:
        song_metadata_result: SongMetadata = await orchestrator.generate_song(
            real_test_prompt
        )
        print(
