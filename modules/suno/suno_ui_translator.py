...
class MockBASDriver:
    ...
    async def input_text(
        ...
        logger.info(
            f"[MockBASDriver] Inputting text into {selector}: 	'{text[:30]}...	'"
        )
    ...
    async def select_option(self, selector: str, value: str):
        logger.info(
            f"[MockBASDriver] Selecting option 	'{value}	' in dropdown: {selector}"
        )
    ...
    async def get_element_text(self, selector: str) -> str:
        ...
        if selector == "#credits_remaining":
            ...
            selector == ".song-list-item a[href*='/song/']"
        ):  # Match selector used below
...
class SunoUITranslator:
    ...
    def __init__(self, bas_driver: Any):
        ...
        self.selectors = {
            ...
            "create_button": "button:contains('Create')"            "generated_song_link": ".song-list-item a[href*=\"/song/\"]",
    ...
    async def execute_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if action_type == "navigate":
                ...
                if not selector:
                    raise SunoUITranslatorError(
                        f"Missing or invalid 'target' key: {target_key}"
                    )
    ...
    async def extract_final_output(
        ...
        try:
            ...
            if song_url and "suno.com/song/" in song_url:
                ...
                logger.info(
                    f"Extracted final output: URL={output['suno_song_url']}, ID={output['suno_song_id']}"
                )
            else:
                logger.error(
                    f"Could not find or parse valid song URL using selector: {song_link_selector}"
                )
