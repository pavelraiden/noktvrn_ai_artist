...
from ...ai_artist_system.noktvrn_ai_artist.artist_builder.prompt_adaptation.prompt_adaptation_pipeline import (
    PromptAdaptationPipeline,
)
...
async def test_adapt_prompt_success(prompt_adapter, mock_llm_orchestrator):
    ...
    context = {"mood": "gloomy"}
    adapted_prompt_llm_output = (
        "Write a melancholic song about a lonely cat watching the rain."
    )
...
async def test_adapt_prompt_llm_fails(prompt_adapter, mock_llm_orchestrator):
    ...
    result = await prompt_adapter.adapt_artist_prompt(
        original_prompt=original_prompt, adaptation_instructions=instructions
    )
...
@pytest.mark.asyncio
async def test_adapt_prompt_with_different_params(
    prompt_adapter, mock_llm_orchestrator
):
