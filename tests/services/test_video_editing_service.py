...
try:
    ...
    if FFMPEG_BINARY == "ffmpeg-imageio": # Default if auto-detect fails
        ...
        if not ffmpeg_path:
            ...
            pytest.skip("Skipping video editing tests: ffmpeg binary not found.", allow_module_level=True)
        else:
    ...
    MOVIEPY_AVAILABLE = False
    pytest.skip("Skipping video editing tests: moviepy library not found.", allow_module_level=True)
except Exception: # Catch potential errors during ffmpeg check
     MOVIEPY_AVAILABLE = False
     pytest.skip("Skipping video editing tests: Error checking ffmpeg availability.", allow_module_level=True)
...
def setup_test_environment():
    ...
    if not os.path.exists(DUMMY_VIDEO_INPUT):
         pytest.skip(f"Skipping video editing tests: Dummy input video not found at {DUMMY_VIDEO_INPUT}", allow_module_level=True)
...
def test_add_text_overlay_invalid_output_dir(video_service):
    ...
    with pytest.raises(Exception): # Catch broader exception as moviepy might raise various things
        video_service.add_text_overlay(
            input_video_path=DUMMY_VIDEO_INPUT,
            output_video_path=output_path,
            text="Error Test",
