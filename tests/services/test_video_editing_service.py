...
try:
    ...
    try:
        ...
        if ffmpeg_binary == "ffmpeg-imageio" or not shutil.which(ffmpeg_binary):
            logger.warning("MoviePy default ffmpeg not found, searching PATH...")
            ...
            if ffmpeg_path_in_path:
                ...
                try:
                    ...
                    logger.warning(f"Failed to set ffmpeg path in MoviePy config: {e_cfg}")
                    # Proceed, hoping MoviePy finds it anyway
                    FFMPEG_AVAILABLE = True # Assume available if found in PATH
            else:
                SKIP_REASON = "ffmpeg binary not found in PATH or MoviePy config."
                logger.error(SKIP_REASON)
        else:
            FFMPEG_AVAILABLE = True
            logger.info(f"ffmpeg binary configured in MoviePy: {ffmpeg_binary}")
...
def setup_test_environment():
    ...
    if not os.path.exists(DUMMY_VIDEO_INPUT):
        if not FFMPEG_AVAILABLE:
             pytest.skip(f"Skipping video tests: Cannot create dummy video, {SKIP_REASON}", allow_module_level=True)
        logger.warning(f"Dummy input video not found at {DUMMY_VIDEO_INPUT}. Creating...")
        try:
...
def test_add_text_overlay_success():
    ...
    logger.info(f"Test 'test_add_text_overlay_success' passed. Output: {output_path}")
...
def test_add_text_overlay_different_position():
    ...
    logger.info(f"Test 'test_add_text_overlay_different_position' passed. Output: {output_path}")
...
def test_add_text_overlay_invalid_output_dir():
    ...
    logger.info(f"Test 'test_add_text_overlay_invalid_output_dir' passed (Caught expected exception: {excinfo.type.__name__}).")
    assert not os.path.exists(output_path)
