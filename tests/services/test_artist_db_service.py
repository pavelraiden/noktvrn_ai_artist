...
def test_add_and_get_artist(db_service):
    ...
    artist_data = {
        ...
        "created_at": now_iso,
        "performance_history": [{"run_id": "run0", "status": "init", "timestamp": now_iso}],
        "is_active": True
    }
...
def test_get_all_artists(db_service):
    ...
    now_iso = datetime.utcnow().isoformat()
    artist1 = {"artist_id": "all1", "name": "Artist One", "created_at": now_iso, "is_active": True}
    artist2 = {"artist_id": "all2", "name": "Artist Two", "created_at": now_iso, "is_active": False}
    artist3 = {"artist_id": "all3", "name": "Artist Three", "created_at": now_iso, "is_active": True}
...
def test_update_artist(db_service):
    ...
    now_iso = datetime.utcnow().isoformat()
    initial_data = {"artist_id": artist_id, "name": "Initial Name", "genre": "initial", "created_at": now_iso}
...
def test_update_artist_performance_approved(db_service):
    ...
    now_iso = datetime.utcnow().isoformat()
    initial_data = {"artist_id": artist_id, "name": "Perf Approve", "created_at": now_iso, "consecutive_rejections": 2}
    db_service.add_artist(initial_data)

    updated = db_service.update_artist_performance_db(artist_id, "run_approve", "approved", 3)
    assert updated is True
...
def test_update_artist_performance_rejected(db_service):
    ...
    now_iso = datetime.utcnow().isoformat()
    initial_data = {"artist_id": artist_id, "name": "Perf Reject", "created_at": now_iso, "consecutive_rejections": 1}
    db_service.add_artist(initial_data)

    updated = db_service.update_artist_performance_db(artist_id, "run_reject", "rejected", 3)
    assert updated is True
...
def test_update_artist_performance_retirement(db_service):
    ...
    initial_data = {"artist_id": artist_id, "name": "Perf Retire", "created_at": now_iso, "consecutive_rejections": retirement_threshold - 1}
    db_service.add_artist(initial_data)

    # Last rejection to trigger retirement
    updated = db_service.update_artist_performance_db(artist_id, "run_retire", "rejected", retirement_threshold)
    assert updated is True
    ...
    updated_again = db_service.update_artist_performance_db(artist_id, "run_after_retire", "rejected", retirement_threshold)
    assert updated_again is True
    retrieved_again = db_service.get_artist(artist_id)
    assert retrieved_again["is_active"] == 0
    assert retrieved_again["consecutive_rejections"] == retirement_threshold + 1 # Count still increments
...
def test_update_artist_performance_history_limit(db_service):
    ...
    initial_data = {"artist_id": artist_id, "name": "Perf History", "created_at": now_iso}
    db_service.add_artist(initial_data)
    ...
    assert retrieved["performance_history"][-1]["run_id"] == f"run_{db_service.MAX_HISTORY_LENGTH + 1}"
