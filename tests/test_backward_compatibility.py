...
class TestBackwardCompatibility(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(
            os.path.join(self.temp_dir, "artists", self.legacy_artist_slug),
            exist_ok=True,
        )
        ...
        with open(
            os.path.join(
                self.temp_dir, "artists", self.legacy_artist_slug, "profile.json"
            ),
            "w",
        ) as f:
        ...
        os.makedirs(
            os.path.join(self.temp_dir, "artists", self.v2_artist_slug), exist_ok=True
        )

        self.v2_profile = {
            "name": "V2 Artist",
            "genre": {"main": "Pop", "subgenres": ["Synth-pop", "Electropop"]},
            "style_tags": ["Upbeat", "Melodic", "Catchy"],
            "vibe": "Energetic and optimistic",
            "target_audience": "Young adults who enjoy dancing",
            "metadata": {"created_at": "2024-06-15T12:00:00Z", "version": 2},
        }

        with open(
            os.path.join(self.temp_dir, "artists", self.v2_artist_slug, "profile.json"),
            "w",
        ) as f:
        ...
        self.trend_analyzer = TrendAnalyzer(
            config={"cache_dir": os.path.join(self.temp_dir, "cache")}
        )
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.temp_dir,
                    "artists",
                    self.legacy_artist_slug,
                    "versions",
                    f"profile_v{result['new_version']}.json",
                )
            )
        )
    ...
    def test_analytics_with_legacy_profiles(self, mock_trend_analyzer):
        ...
        mock_trend_analyzer_instance.analyze_artist_compatibility.return_value = {
            "overall_score": 0.8,
            "aspect_scores": {"genre": 0.9, "style": 0.7},
        }
    ...
    def test_mixed_version_artist_comparison(self):
        ...
        # Mock trend analyzer
        with patch(
            "artist_builder.artist_analytics.TrendAnalyzer"
        ) as mock_trend_analyzer:
            mock_trend_analyzer_instance = MagicMock()
            mock_trend_analyzer_instance.analyze_trends.return_value = {}
            mock_trend_analyzer_instance.analyze_artist_compatibility.return_value = {
                "overall_score": 0.7,
    ...
    def test_api_backward_compatibility(self):
        ...
        with patch(
            "artist_builder.artist_interface.ArtistProfileBuilder"
        ) as mock_builder:
            mock_builder_instance = MagicMock()
            mock_builder_instance.build_profile.return_value = {
                "name": "API Test Artist",
                "genre": {"main": "Electronic", "subgenres": ["Ambient", "Downtempo"]},
...
class TestFileSystemCompatibility(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(
            os.path.join(self.temp_dir, "artists", self.legacy_artist_slug, "assets"),
            exist_ok=True,
        )
        os.makedirs(
            os.path.join(self.temp_dir, "artists", self.legacy_artist_slug, "songs"),
            exist_ok=True,
        )
    ...
    def test_file_structure_preservation(self, mock_orchestrator):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.temp_dir, "artists", self.legacy_artist_slug, "assets"
                )
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.temp_dir, "artists", self.legacy_artist_slug, "versions"
                )
            )
        )
    ...
    def test_new_directory_creation(self, mock_orchestrator):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.temp_dir, "artists", self.legacy_artist_slug, "prompts"
                )
            )
        )
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    self.temp_dir, "artists", self.legacy_artist_slug, "lyrics"
                )
            )
        )
...
class TestConfigurationCompatibility(unittest.TestCase):
    ...
    def test_legacy_config_loading(self):
        ...
        with patch("artist_builder.artist_interface.open", create=True) as mock_open:
            ...
            self.assertEqual(
                interface.config.get("artists_dir"),
                self.legacy_config["artists_directory"],
            )
            self.assertEqual(
                interface.config.get("llm_api_key"), self.legacy_config["llm_api_key"]
            )
            self.assertEqual(
                interface.config.get("llm_model"), self.legacy_config["llm_model"]
            )

    @patch(
        "artist_builder.artist_interface.os.environ",
        {
