...
class TestReleaseChain(unittest.TestCase):
    ...
    def setUp(self):
        ...
        release_chain.RELEASE_QUEUE_FILE = os.path.join(self.test_dir, "release_queue.json")
        ...
        with open(release_chain.RELEASE_LOG_FILE, "w") as f: f.write("")
        with open(release_chain.RELEASE_QUEUE_FILE, "w") as f: f.write("[]")
    ...
    def test_generate_artist_slug(self):
        self.assertEqual(release_chain.generate_artist_slug("Synthwave Dreamer"), "synthwave_dreamer")
        self.assertEqual(release_chain.generate_artist_slug("Artist Name!"), "artist_name")
        self.assertEqual(release_chain.generate_artist_slug("  Multiple   Spaces  "), "multiple_spaces")
    ...
    def test_add_release_to_queue_success_empty(self, mock_dump, mock_load, mock_file):
        metadata = MockReleaseMetadata(release_id="test_q_1", artist_name="Queue Artist")
        ...
        self.assertEqual(dumped_data[0]["release_directory"], str(release_dir_path.resolve()))

    @patch("builtins.open", new_callable=mock_open)
    @patch("json.load", return_value=[{"release_id": "existing_1"}])
    @patch("json.dump")
    def test_add_release_to_queue_success_existing(self, mock_dump, mock_load, mock_file):
        metadata = MockReleaseMetadata(release_id="test_q_2", artist_name="Queue Artist 2")
    ...
    def test_process_approved_run_success(self, mock_add_queue, mock_log_md, mock_save_prompts, 
                                        mock_save_meta, mock_get_prompts, mock_analyze_track, 
                                        mock_gen_cover, mock_dl_asset, mock_create_dir):
        run_id = "proc_ok_1"
        artist_name = "Processor Artist"
        artist_slug = "processor_artist"
        date_str = datetime.utcnow().strftime("%Y%m%d")
        release_id = f"{artist_slug}_{date_str}_{run_id[:4]}"
        mock_release_dir = Path(self.test_dir) / "releases" / f"{artist_slug}_{date_str}"
        ...
        run_data = {"run_id": run_id, "status": "approved", "artist_name": artist_name, "track_url": "t_url", "video_url": "v_url"}
    ...
    @patch("release_chain.download_asset", return_value=False) # Simulate asset download failure
    def test_process_approved_run_asset_failure(self, mock_dl_asset, mock_create_dir):
        run_id = "proc_asset_fail"
        run_data = {"run_id": run_id, "status": "approved", "artist_name": "Fail Artist"}
        run_status_filepath = Path(release_chain.RUN_STATUS_DIR) / f"run_{run_id}.json"
        with open(run_status_filepath, "w") as f:
            json.dump(run_data, f)
        success = release_chain.process_approved_run(run_id)
