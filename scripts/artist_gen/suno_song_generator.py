...
class SunoSongGenerator:
    ...
    def generate_song(
        ...
        if output_dir is None:
            ...
            output_dir = DEFAULT_OUTPUT_DIR_TEMPLATE.format(
    ...
    def _wait_for_completion(
        ...
        while time.time() - start_time < max_wait_time:
            ...
            status_file = (
                output_path / f"{self._sanitize_filename(title)}_status.json"
            )
...
def generate_song_from_artist_profile(
    ...
    if not genre:
        try:
            ...
            logger.warning(
                f"Could not load genre from artist profile: {str(e)}"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate songs using Suno AI"
    )
    parser.add_argument(
        "--artist", required=True, help="Artist slug/identifier"
    )
    parser.add_argument("--title", required=True, help="Song title")
    parser.add_argument(
        "--prompt", required=True, help="Song prompt/description"
    )
