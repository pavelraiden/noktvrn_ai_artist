...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/suno_api.log"),
...
class SunoSongGenerator:
    ...
    def generate_song(self, title, prompt, artist_slug, genre=None, 
                      reference_song=None, bpm=None, duration=None, 
                      output_dir=None, wait_for_completion=False, 
                      check_interval=10, max_wait_time=300):
        ...
        try:
            ...
            with open(request_file, 'w') as f:
                json.dump({
                    "request": payload,
                    "initial_response": generation_data
                }, f, indent=2)
    ...
    def check_generation_status(self, generation_id):
        ...
        try:
            response = requests.get(
                f"{SUNO_STATUS_ENDPOINT}/{generation_id}",
                headers=self.headers
            )
            response.raise_for_status()
    ...
    def _wait_for_completion(self, generation_id, output_path, title, check_interval, max_wait_time):
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if "error" in status_data:
                logger.error(f"Error while waiting for completion: {status_data['error']}")
                return status_data
...
def generate_song_from_artist_profile(artist_slug, song_title, song_prompt, 
                                      genre=None, wait_for_completion=False):
