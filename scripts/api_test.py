...
TEST_VIDEO_KEYWORDS = ["retro car race", "city night lights", "electronic music concert"]
...
def test_pixabay_api():
    ...
    try:
        ...
        for keyword in TEST_VIDEO_KEYWORDS:
            ...
            if video_url:
                logger.info(f"Successfully fetched video from Pixabay for keyword: {keyword}")
                results[keyword] = {
                    "status": "success",
                    "url": video_url
                }
...
def test_pexels_api():
    ...
    try:
        ...
        for keyword in TEST_VIDEO_KEYWORDS:
            ...
            if video_url:
                logger.info(f"Successfully fetched video from Pexels for keyword: {keyword}")
                results[keyword] = {
                    "status": "success",
                    "url": video_url
                }
...
def run_all_tests():
    ...
    success_count = sum(1 for api, result in all_results.items() if result["status"] == "success")
    partial_success_count = sum(1 for api, result in all_results.items() if result["status"] == "partial_success")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--api", choices=["suno", "pixabay", "pexels", "all"], default="all",
