    ...
    try:
        ...
        if os.path.exists(status_filepath):
            try:
                ...
                logger.error(
                    f"Error decoding existing status file {status_filepath}, will overwrite: {e}"
                )
            except IOError as e:
                logger.error(
                    f"Error reading existing status file {status_filepath}, will overwrite: {e}"
                )
        else:
            logger.warning(
                f"Status file {status_filepath} not found for update, creating basic structure."
            )
        ...
        logger.info(
            f"Updated status for run {run_id} to '{new_status}' in {status_filepath}"
        )

    except Exception as e:
        logger.error(
            f"Failed to update status file {status_filepath}: {e}", exc_info=True
        )


def create_initial_run_status(
    run_id, artist_profile, parameters, track_info, video_info
):
...
def send_to_telegram_for_approval(
    ...
    try:
        ...
        if success:
            ...
            logger.error(
                f"Failed to send run {run_id} to Telegram (telegram_service returned False)."
            )
...
def check_approval_status(run_id):
    ...
    try:
        if not os.path.exists(status_filepath):
            logger.warning(
                f"Status file not found for run {run_id}: {status_filepath}. Assuming pending."
            )
            return None
        ...
        if current_status == "approved":
            ...
            logger.warning(
                f"Run {run_id} has unexpected status: {current_status}. Treating as rejected."
            )
            return False

    except json.JSONDecodeError as e:
        logger.error(
            f"Error decoding status file {status_filepath}: {e}. Treating as pending."
        )
        return None
    except IOError as e:
        logger.error(
            f"Error reading status file {status_filepath}: {e}. Treating as pending."
        )
        return None
    except Exception as e:
        logger.error(
            f"Unexpected error checking status file {status_filepath}: {e}",
            exc_info=True,
        )
        return None
...
def save_approved_content(run_id):
    ...
    try:
        ...
        if run_data.get("status") == "approved":
            ...
            logger.warning(
                f"Attempted to save content for run {run_id}, but status is not 'approved' ({run_status})."
            )
            return False
    except Exception as e:
        logger.error(
            f"Error processing approved content for run {run_id}: {e}", exc_info=True
        )
        return False
...
def trigger_release_logic(run_id):
    ...
    try:
        ...
        logger.error(
            f"Exception during release chain processing for run {run_id}: {e}",
            exc_info=True,
        )
...
def run_batch_cycle():
    ...
    try:
        ...
        logger.info(
            f"Waiting up to {MAX_APPROVAL_WAIT_TIME}s for approval for run {run_id}..."
        )
        ...
        while time.time() - start_wait < MAX_APPROVAL_WAIT_TIME:
            ...
            logger.warning(
                f"Timeout waiting for approval for run {run_id}. Assuming rejected."
            )
        ...
        logger.warning(
            f"Keyboard interrupt received during run {run_id}. Shutting down."
        )
        update_run_status(run_id, "aborted", "Keyboard interrupt")
        raise
    except Exception as e:
        logger.critical(
            f"Critical unexpected error during batch cycle run {run_id}: {e}",
            exc_info=True,
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        "NOTE: This script relies on an external webhook server to update run status files."
    )
    try:
        run_batch_cycle()
    except KeyboardInterrupt:
