...
class SunoApiClient(BaseApiClient):
    ...
    def __init__(self, api_key: str | None = None, base_url: str | None = None):
        ...
        Args:
            api_key: The Suno API key. Defaults to SUNO_API_KEY environment variable.
            base_url: The base URL for the Suno API. Defaults to DEFAULT_BASE_URL or SUNO_BASE_URL environment variable.
        """
    ...
    def start_audio_generation(
        ...
        try:
            ...
            logger.info(
                f"Suno music generation task started successfully. Response: {response_data}"
            )
            ...
            if not isinstance(response_data, list) or not all(
                ...
                raise SunoApiError(
                    f"Suno API returned an unexpected response format: {response_data}"
                )
    ...
    def get_generation_details(self, clip_ids: list[str]) -> list:
        ...
        try:
            ...
            if not isinstance(response_data, list):
                raise SunoApiError(
                    f"Suno API returned an unexpected response format when fetching details: {response_data}"
                )
            return response_data
        except ApiClientError as e:
            logger.error(f"Suno API request failed: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    if not suno_api_key:
        print(
            "Please ensure the SUNO_API_KEY is set in your .env file or environment variables to run this example."
        )
    else:
        try:
            ...
            if clip_ids:
                ...
                while attempts < max_attempts:
                    ...
                    if all_complete:
                        ...
                        print(
                            f"Statuses: {current_statuses}. Waiting {wait_time} seconds..."
                        )
                ...
                if not final_details:
                    ...
                    for item in final_details:
                        print(
                            f"Clip {item.get('id')} Audio URL: {item.get('audio_url')}"
                        )
                else:
