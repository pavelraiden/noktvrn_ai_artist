...
# We'll retry on generic connection/timeout errors and potentially \
# PexelsApiError if it wraps transient issues.
# If the library raises standard requests exceptions, add them here \
# (e.g., requests.exceptions.RequestException)
...
class PexelsClient:
    """Client for interacting with the Pexels API to search for stock \
    videos."""
    ...
    def __init__(self):
        ...
        if not self.api_key:
            ...
            # Raise config error immediately, no point retrying \
            # initialization
            raise ValueError("PEXELS_API_KEY is required but not set.")
        try:
            # Add timeout to the Pexels client if the library supports it \
            # (check library docs/source)
            # Assuming it doesn't have a direct timeout param, rely on \
            # underlying HTTP library timeouts or decorator
            self.client = Pexels(self.api_key)
            logger.info("Pexels client initialized successfully.")
        except Exception as e:
            logger.error(
                f"Failed to initialize Pexels client: {e}", exc_info=True
            )
            # Wrap initialization error, but don't use PexelsApiError here
            # as it's for API call issues
    ...
    def search_videos(
        ...
        page: int = 1,
        per_page: int = 15,
        min_duration: int | None = None,  # Keep params even if unused by lib
        ...
        Args:
            ...
            min_duration: Minimum duration in seconds
                          (Note: May not be supported by underlying library).
        ...
        Raises:
            PexelsApiError: If the API call fails definitively after
                          retries.
            Exception: Other exceptions if they occur and are not in \
                       PEXELS_RETRY_EXCEPTIONS.
        """
        logger.info(
            f"Searching Pexels videos for query: 	{query}	 "
            f"with params: orientation={orientation}, size={size}, "
        ...
        # The library might return a dict-like object, let's ensure it's a
        # standard dict
        # The pexelsapi library seems to return a PexelsResult object,
        # convert it
        try:
            ...
            logger.error(
                "Failed to parse Pexels response structure: %s. Response: %s",
                e,
                str(search_results)[:100] + "...",
            )
        ...
        logger.info(
            f"Found {results_dict.get('total_results', 0)} videos "
            f"for query: '{query}'"
        )
        return results_dict
...
if __name__ == "__main__":
    ...
    try:
        ...
        if pexels_client.api_key:
            ...
            try:
                videos = pexels_client.search_videos(
                ...
                if videos.get("videos"):
                    ...
                    if video_files:
                        # Ensure width/height exist and handle potential \
                        # None values
                        valid_files = [
                            f
                        ...
                        if valid_files:
                            ...
                            print(
                                f"Highest quality video link "
                                f"({hq_link.get('quality')}): "
                                f"{hq_link.get('link')}"
                            )
                        else:
                            print(
                                "No video files with valid dimensions found."
                            )
                    else:
                ...
                print(
                    f"\nError during Pexels API call for 	{search_query}	: "
                    f"{api_call_error}"
                )
        else:
            # This case should ideally not happen if ValueError is raised \
